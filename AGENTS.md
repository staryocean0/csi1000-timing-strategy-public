...
云端协作已由用户启用。本任务沿用户明确的公库标准runner计算、私库存储设计；只有人工workflow_dispatch，私库Actions关闭。不添加push/PR/fork私密任务，不使用付费larger runner/GPU，不使用公开artifact/cache/summary存数据、源码、研究结果、异常堆栈或私有日志。
