...
PROFILE_NAME = "ols-maxdd-failure-atlas-v1"
PROFILE_SHA256 = "1118ba4699c3ed61992ea05d3380f860ced93845d362a54198c7b1c790856daa"
PROFILE = {
    "private_ref": d0b.PRIVATE_REF,
    "manifest_sha256": PROFILE_SHA256,
    "command": ["d0/ols_maxdd_failure_atlas_v1.py", "--inputs", "/work/inputs", "--out", "/results/study"],
    "verify_command": ["d0/ols_maxdd_failure_atlas_v1_verifier.py", "--inputs", "/work/inputs", "--results", "/results/study"],
...
_SAFE_CODE = re.compile(r"\bols_maxdd_atlas_[a-z0-9_:-]+\b")
...
def require_context() -> None:
    ...
    if env.get("GITHUB_ACTIONS") != "true" or env.get("GITHUB_REPOSITORY") != rb.PUBLIC_REPO or env.get("GITHUB_EVENT_NAME") != "workflow_dispatch" or env.get("GITHUB_REF") != "refs/heads/cloud-workspace-v1":
        raise GateError("not_approved_ols_maxdd_atlas_context")
...
def prepare_inputs(api, root: Path, profile: dict) -> Path:
    profile_path = HERE / "ols_maxdd_failure_atlas_v1_profile.json"
    if not profile_path.is_file() or profile_path.is_symlink() or _sha256(profile_path) != PROFILE_SHA256:
        raise GateError("ols_maxdd_atlas_profile_identity_failed")
    for name, expected in PINNED_D0_BLOBS.items():
        ...
        if not source.is_file() or source.is_symlink() or _blobsha(source.read_bytes()) != expected:
            raise GateError(f"ols_maxdd_atlas_d0_identity_failed_{name}")
    work = d0b.prepare_inputs(api, root, d0b.PROFILE)
    d0dir = work / "d0"
    for name in ("ols_maxdd_failure_atlas_v1.py", "ols_maxdd_failure_atlas_v1_verifier.py"):
        source = HERE / name
        if not source.is_file() or source.is_symlink():
            raise GateError("ols_maxdd_atlas_public_source_missing")
...
def _safe_failure_diagnostic() -> None:
    try:
        ...
        print('OLS_MAXDD_ATLAS_SAFE_DIAGNOSTIC={"available":false,"reason":"state_unavailable"}', file=sys.stderr)
        return
    ...
    print("OLS_MAXDD_ATLAS_SAFE_DIAGNOSTIC=" + json.dumps({"available": bool(codes or exceptions), "compute_exit_code": state.get("compute_exit_code"), "validation_exit_code": state.get("validation_exit_code"), "codes": sorted(codes), "exception_classes": sorted(exceptions)}, sort_keys=True, separators=(",", ":")), file=sys.stderr)


def _mirror_private_text_results() -> None:
    state, root = rb.load_state()
    if not state.get("compute_success") or not state.get("cleanup_complete"):
        raise GateError("ols_maxdd_atlas_private_text_mirror_requires_verified_success")
    ...
    for relative in _PRIVATE_TEXT_MIRROR:
        ...
        if not source.is_file() or source.is_symlink() or source.stat().st_size > 256 * 1024:
            raise GateError("ols_maxdd_atlas_private_text_mirror_source_invalid")
        raw = source.read_bytes(); raw.decode("utf-8")
        private_path = f"research/public-runs/{state['run_id']}/ols-maxdd-atlas/{relative}"
        endpoint = f"repos/{rb.PRIVATE_REPO}/contents/{urllib.parse.quote(private_path, safe='/')}"
        api.request(endpoint, {"message": "Mirror verified OLS MaxDD atlas text result [skip ci]", "branch": state["branch"], "content": base64.b64encode(raw).decode()}, method="PUT")
        ...
        if returned.get("encoding") != "base64" or base64.b64decode(returned.get("content", "") or "") != raw:
            raise GateError("ols_maxdd_atlas_private_text_mirror_readback_failed")
    print("Verified OLS MaxDD atlas text mirror written to the private run branch.")
...
def main() -> None:
    ...
    require_context()
    if args.profile != PROFILE_NAME: raise GateError("unknown_ols_maxdd_atlas_profile")
...
def run() -> None:
    try: main()
        ...
        print("Execution failed; no private OLS MaxDD atlas content was exposed publicly.", file=sys.stderr); raise SystemExit(1)
