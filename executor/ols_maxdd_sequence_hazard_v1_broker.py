...
PROFILE_NAME = "ols-maxdd-sequence-hazard-v1"
PROFILE_SHA256 = "71fe62797e9ec9f9f106e313b1adcbd5424915c9ed9d51ceb0a06319c8bf4282"
PROFILE = {
    "private_ref": d0b.PRIVATE_REF,
    "manifest_sha256": PROFILE_SHA256,
    "command": ["d0/ols_maxdd_sequence_hazard_v1.py", "--inputs", "/work/inputs", "--out", "/results/study"],
    "verify_command": ["d0/ols_maxdd_sequence_hazard_v1_verifier.py", "--inputs", "/work/inputs", "--results", "/results/study"],
...
_PRIVATE_TEXT_MIRROR = (
    ...
    "study/seq3_sensitivity.csv",
    "study/RESULTS.md",
)
_SAFE_CODE = re.compile(r"\bols_maxdd_sequence_[a-z0-9_:-]+\b")
...
def require_context() -> None:
    ...
    if env.get("GITHUB_ACTIONS") != "true" or env.get("GITHUB_REPOSITORY") != rb.PUBLIC_REPO or env.get("GITHUB_EVENT_NAME") != "workflow_dispatch" or env.get("GITHUB_REF") != "refs/heads/cloud-workspace-v1":
        raise GateError("not_approved_ols_maxdd_sequence_context")
...
def prepare_inputs(api, root: Path, profile: dict) -> Path:
    profile_path = HERE / "ols_maxdd_sequence_hazard_v1_profile.json"
    if not profile_path.is_file() or profile_path.is_symlink() or _sha256(profile_path) != PROFILE_SHA256:
        raise GateError("ols_maxdd_sequence_profile_identity_failed")
    for name, expected in PINNED_D0_BLOBS.items():
        ...
        if not source.is_file() or source.is_symlink() or _blobsha(source.read_bytes()) != expected:
            raise GateError("ols_maxdd_sequence_d0_identity_failed_" + name)
    work = d0b.prepare_inputs(api, root, d0b.PROFILE)
    d0dir = work / "d0"
    for name in ("ols_maxdd_sequence_hazard_v1.py", "ols_maxdd_sequence_hazard_v1_verifier.py"):
        source = HERE / name
        if not source.is_file() or source.is_symlink():
            raise GateError("ols_maxdd_sequence_public_source_missing")
...
def _safe_failure_diagnostic() -> None:
    try:
        ...
        print('OLS_MAXDD_SEQUENCE_SAFE_DIAGNOSTIC={"available":false,"reason":"state_unavailable"}', file=sys.stderr)
        return
    ...
    print("OLS_MAXDD_SEQUENCE_SAFE_DIAGNOSTIC=" + json.dumps({"available": bool(codes or exceptions), "compute_exit_code": state.get("compute_exit_code"), "validation_exit_code": state.get("validation_exit_code"), "codes": sorted(codes), "exception_classes": sorted(exceptions)}, sort_keys=True, separators=(",", ":")), file=sys.stderr)


def _mirror_private_text_results() -> None:
    state, root = rb.load_state()
    if not state.get("compute_success") or not state.get("cleanup_complete"):
        raise GateError("ols_maxdd_sequence_private_text_mirror_requires_verified_success")
    ...
    for relative in _PRIVATE_TEXT_MIRROR:
        ...
        if not source.is_file() or source.is_symlink() or source.stat().st_size > 256 * 1024:
            raise GateError("ols_maxdd_sequence_private_text_mirror_source_invalid")
        raw = source.read_bytes(); raw.decode("utf-8")
        private_path = f"research/public-runs/{state['run_id']}/ols-maxdd-sequence/{relative}"
        endpoint = f"repos/{rb.PRIVATE_REPO}/contents/{urllib.parse.quote(private_path, safe='/')}"
        api.request(endpoint, {"message": "Mirror verified OLS MaxDD sequence-hazard text result [skip ci]", "branch": state["branch"], "content": base64.b64encode(raw).decode()}, method="PUT")
        ...
        if returned.get("encoding") != "base64" or base64.b64decode(returned.get("content", "") or "") != raw:
            raise GateError("ols_maxdd_sequence_private_text_mirror_readback_failed")
    print("Verified OLS MaxDD sequence-hazard text mirror written to the private run branch.")
...
def main() -> None:
    ...
    if args.profile != PROFILE_NAME:
        raise GateError("unknown_ols_maxdd_sequence_profile")
    rb.prepare_inputs = prepare_inputs
    if args.phase == "prepare": rb.prepare(PROFILE_NAME, PROFILE)
    elif args.phase == "compute":
        try: rb.compute()
        except GateError as error:
            if str(error) == "compute_failed_private_publish_step_will_report": _safe_failure_diagnostic()
            raise
...
def run() -> None:
    try: main()
        ...
        print("Execution failed; no private OLS MaxDD sequence-hazard content was exposed publicly.", file=sys.stderr); raise SystemExit(1)
