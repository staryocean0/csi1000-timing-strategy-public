...
PROFILE_NAME = "ols-r2-d1-leadlag-v1"
PRIVATE_REF = "67effb80f51228f6129dca5c4f7971a0bb6c7f15"
DATA_REF = "1d760ea9525eb3688b70a4aa0f2b5b207af16a17"
D0_PUBLIC_SOURCE_COMMIT = "d4f12c879c12bd63ab4092a4e57a515880e2f77e"
PROFILE_SHA256 = "50a0929364401269f7c852ece205b3280950c5242465794ec0aa0d98a497f8df"
DATA = {
...
_SAFE_D1_CODE = re.compile(r"\bols_d1_[a-z0-9_:-]+\b")
_SAFE_EXCEPTION = re.compile(r"^([A-Za-z_][A-Za-z0-9_.]*(?:Error|Exception))(?::|$)", re.MULTILINE)
_SAFE_LOCATION = re.compile(r'File "/work/d1/(ols_r2_d1(?:_verifier)?\.py|ols_drawdown_d0(?:_session_adapter)?\.py)", line ([0-9]+)')
_PRIVATE_TEXT_MIRROR = (
    "study/RESULTS.json",
    "study/mode_summary.csv",
    "study/qualification_reset_top20_leadlag.csv",
    "study/first_opposite_close_top20_leadlag.csv",
    "study/two_opposite_closes_top20_leadlag.csv",
    "study/prior_extreme_break_top20_leadlag.csv",
    "study/frozen_midline_break_top20_leadlag.csv",
)
...
def require_context() -> None:
    ...
    if (
        ...
        raise GateError("not_approved_ols_d1_context")
...
def fetch_public(year: int, target: Path) -> None:
    ...
    with urllib.request.urlopen(request, timeout=60) as response:
        if urllib.parse.urlparse(response.geturl()).hostname != "raw.githubusercontent.com":
            raise GateError("ols_d1_public_redirect_rejected")
        raw = response.read(size + 1)
    if len(raw) != size or blobsha(raw) != expected_blob:
        raise GateError(f"ols_d1_public_identity_failed_{year}")
...
def fetch_private_source(api, path: str, expected_blob: str, root: Path) -> dict[str, object]:
    ...
    if meta.get("type") != "file" or meta.get("encoding") != "base64" or meta.get("sha") != expected_blob:
        raise GateError("ols_d1_private_source_identity_failed")
    try:
        raw = base64.b64decode(meta.get("content", "") or "")
    except Exception:
        raise GateError("ols_d1_private_source_decode_failed") from None
    if blobsha(raw) != expected_blob:
        raise GateError("ols_d1_private_source_blob_failed")
    target = root / path
...
def prepare_inputs(api, root: Path, profile: dict) -> Path:
    ...
    profile_path = HERE / "ols_r2_d1_profile.json"
    if not profile_path.is_file() or profile_path.is_symlink() or sha256(profile_path) != PROFILE_SHA256:
        raise GateError("ols_d1_profile_identity_failed")
    for name in (
        "ols_drawdown_d0.py",
        "ols_drawdown_d0_session_adapter.py",
        "ols_r2_d1.py",
        "ols_r2_d1_verifier.py",
    ):
        source = HERE / name
        if not source.is_file() or source.is_symlink():
            raise GateError("ols_d1_public_source_missing")
        shutil.copy2(source, d1 / name)
    ...
    provenance = {
        "schema_id": "ols_r2_d1_source_provenance@1.0",
        "private_ref": PRIVATE_REF,
        "source_blobs": SOURCE_BLOBS,
        "source_receipt": private_receipt,
        "d0_public_source_commit": D0_PUBLIC_SOURCE_COMMIT,
...
def _safe_failure_diagnostic() -> None:
    try:
        ...
        print('OLS_D1_SAFE_DIAGNOSTIC={"available":false,"reason":"state_unavailable"}', file=sys.stderr)
        return
    ...
    for path in paths:
        ...
        codes.update(_SAFE_D1_CODE.findall(text))
    ...
    print("OLS_D1_SAFE_DIAGNOSTIC=" + json.dumps(payload, sort_keys=True, separators=(",", ":")), file=sys.stderr)


def _mirror_private_text_results() -> None:
    state, root = rb.load_state()
    if not state.get("compute_success") or not state.get("cleanup_complete"):
        raise GateError("ols_d1_private_text_mirror_requires_verified_success")
    ...
    for relative in _PRIVATE_TEXT_MIRROR:
        ...
        if not source.is_file() or source.is_symlink() or source.stat().st_size > 256 * 1024:
            raise GateError("ols_d1_private_text_mirror_source_invalid")
        raw = source.read_bytes()
        try:
            raw.decode("utf-8")
        except UnicodeDecodeError:
            raise GateError("ols_d1_private_text_mirror_not_utf8") from None
        private_path = f"research/public-runs/{state['run_id']}/ols-d1/{relative}"
        endpoint = f"repos/{rb.PRIVATE_REPO}/contents/{urllib.parse.quote(private_path, safe='/')}"
        api.request(
            endpoint,
            {
                "message": "Mirror verified OLS D1 text result [skip ci]",
                "branch": state["branch"],
        ...
        if returned.get("encoding") != "base64" or base64.b64decode(returned.get("content", "") or "") != raw:
            raise GateError("ols_d1_private_text_mirror_readback_failed")
    print("Verified OLS D1 text mirror written to the private run branch.")
...
def main() -> None:
    ...
    if args.profile != PROFILE_NAME:
        raise GateError("unknown_ols_d1_profile")
...
def run() -> None:
    try:
        ...
        print("Execution failed; no private OLS D1 content was exposed publicly.", file=sys.stderr)
        raise SystemExit(1)
