"""Mirror only verified tail-robust calibration text outputs to the private run branch."""
...
FILES = (
    ...
    "ANNUAL_CALIBRATION_V2.csv",
    "TEMPORAL_METRICS_V2.csv",
    "ACCEPTANCE_RESULT_V2.json",
...
def main():
    ...
    if state.get("profile_name") != PROFILE or state.get("compute_success") is not True or state.get("cleanup_complete") is not True:
        raise GateError("tail_robust_state_not_mirrorable")
    ...
    for name in FILES:
        ...
        if path.is_symlink() or not path.is_file() or not 0 < path.stat().st_size <= 512 * 1024:
            raise GateError("tail_robust_mirror_file_invalid")
        raw = path.read_bytes()
        try:
            raw.decode("utf-8")
        except UnicodeDecodeError:
            raise GateError("tail_robust_mirror_file_not_utf8") from None
        target = f"repos/{base.PRIVATE_REPO}/contents/research/public-runs/{state['run_id']}-results/{name}"
        api.request(
            target,
            {
                "message": "Record verified tail-robust calibration result [skip ci]",
                "branch": state["branch"],
        ...
        if base64.b64decode(got["content"]) != raw:
            raise GateError("tail_robust_mirror_readback_failed")
    print("Verified tail-robust calibration outputs mirrored privately.")
