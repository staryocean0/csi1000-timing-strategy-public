...
PROFILE_NAME="risk-v2-15m-annual-tail-decomposition-v1"
PROFILE={"private_ref":tb.PRIVATE_REF,"manifest_sha256":tb.P1[6],"command":["tail/risk_annual_tail_decomposition.py","--inputs","/work/inputs","--out","/results/study"],"verify_command":["tail/risk_annual_tail_decomposition_verifier.py","--inputs","/work/inputs","--results","/results/study"],"command_timeout_seconds":300,"verification_timeout_seconds":300,"new_training":False,"production_authority":False}
rb.COMPUTE_HOST_TIMEOUT_SECONDS=330;rb.VALIDATE_HOST_TIMEOUT_SECONDS=330

def require_context():
    e=os.environ
    if e.get("GITHUB_ACTIONS")!="true" or e.get("GITHUB_REPOSITORY")!=rb.PUBLIC_REPO or e.get("GITHUB_EVENT_NAME")!="workflow_dispatch" or e.get("GITHUB_REF")!="refs/heads/cloud-workspace-v1":raise GateError("not_approved_annual_tail_context")
...
def prepare_inputs(api,root:Path,profile:dict):
    work=tb.prepare_inputs(api,root,profile);inputs=work/"inputs";tail=work/"tail";tail.mkdir();shutil.copy2(HERE/"risk_temporal_stability_2015_2025.py",inputs/"temporal_base.py")
    for name in ("risk_annual_tail_decomposition.py","risk_annual_tail_decomposition_verifier.py"):
        source=HERE/name
        if not source.is_file() or source.is_symlink():raise GateError("annual_tail_source_missing")
        shutil.copy2(source,tail/name)
    return work
...
def main():
    ...
    if a.profile!=PROFILE_NAME:raise GateError("unknown_annual_tail_profile")
...
def run():
    ...
    except Exception:print("Execution failed; no private annual-tail content was exposed publicly.",file=sys.stderr);raise SystemExit(1)
