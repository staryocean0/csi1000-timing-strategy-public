"""Mirror only verified final-confirmatory text outputs to the private run branch."""
...
PROFILE = "risk-v2-final-2026-confirmatory-v1"
FILES = ("SUMMARY.json", "EQUIVALENCE_GUARD.json", "INPUT_DATA_RECEIPT.json")
...
def main() -> None:
    ...
    if state.get("profile_name") != PROFILE or state.get("compute_success") is not True or state.get("cleanup_complete") is not True:
        raise GateError("final_confirmatory_state_not_mirrorable")
    ...
    for name in FILES:
        ...
        if path.is_symlink() or not path.is_file() or not 0 < path.stat().st_size <= 1024 * 1024:
            raise GateError("final_confirmatory_mirror_file_invalid")
        raw = path.read_bytes()
        try:
            raw.decode("utf-8")
        except UnicodeDecodeError:
            raise GateError("final_confirmatory_mirror_file_not_utf8") from None
        target = f"repos/{base.PRIVATE_REPO}/contents/research/public-runs/{state['run_id']}-results/{name}"
        api.request(target, {
            "message": "Record verified final Risk Tool confirmatory result [skip ci]",
            "branch": state["branch"],
            "content": base64.b64encode(raw).decode("ascii"),
        }, method="PUT")
        got = api.request(target + "?ref=" + base.urllib.parse.quote(state["branch"], safe=""))
        if base64.b64decode(got["content"]) != raw:
            raise GateError("final_confirmatory_mirror_readback_failed")
    print("Verified final Risk Tool confirmatory text outputs mirrored privately.")
