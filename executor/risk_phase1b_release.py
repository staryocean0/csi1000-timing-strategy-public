...
HS=(15,30);DEV=(2021,2022,2023);AUD=(2024,2025);SYM=("000688.SH","000852.SH")
REPS=5000;SEED=20260914;LAM=1e-6
...
def fitcal(p,y):
 ...
 for _ in range(100):
  e=X@b;u=np.where(e>=0,1/(1+np.exp(-e)),np.exp(e)/(1+np.exp(e)));w=np.maximum(u*(1-u),1e-12)
  d=np.linalg.solve((X.T*w)@X/len(y)+P,X.T@(u-y)/len(y)+P@b);b-=d
...
def run(inp,out):
 st=inp/"state_rows.parquet";co=inp/"cohort_rows.parquet"
 if not st.is_file() or not co.is_file() or sha(st)!=PSTATE or sha(co)!=PCOH:raise RuntimeError("parent output drift")
 x=pd.read_parquet(co);out.mkdir(parents=True,exist_ok=False);shutil.copy2(st,out/"state_rows.parquet");shutil.copy2(co,out/"cohort_rows.parquet")
 R={};F={}
 for h in HS:
  y=f"normal_within_{h}m";b=f"p_B_{h}m";c=f"p_C_{h}m"
  D=x[x.year.isin(DEV)&x[y].notna()].copy();A=x[x.year.isin(AUD)&x[y].notna()].copy();raw=gain(D,y,b,c);lo=boot(D,y,b,c)
  Y={str(v):gain(D[D.year.eq(v)],y,b,c) for v in DEV};S={s:gain(D[D.symbol.eq(s)],y,b,c) for s in SYM};AY={str(v):gain(A[A.year.eq(v)],y,b,c) for v in AUD};AS={s:gain(A[A.symbol.eq(s)],y,b,c) for s in SYM};ar=gain(A,y,b,c)
  parts=[];cal={}
  for v in DEV:
  ...
  pg=[oo,cg["brier_gain"]>0,cg["logloss_gain"]>0,sum(gain(Z[Z.year.eq(v)],y,"cb","cc")["brier_gain"]>=0 and gain(Z[Z.year.eq(v)],y,"cb","cc")["logloss_gain"]>=0 for v in DEV)>=2,ca["brier_gain"]>=0,ca["logloss_gain"]>=0,all(CAY[str(v)]["brier_gain"]>=0 and CAY[str(v)]["logloss_gain"]>=0 for v in AUD),all(CAS[s]["brier_gain"]>=0 and CAS[s]["logloss_gain"]>=0 for s in SYM)];po=all(pg)
  status="ORDERING_AND_CALIBRATION_SUPPORTED_PENDING_FRESH_OOS" if oo and po else ("ORDERING_ONLY_CALIBRATION_NOT_SUPPORTED" if oo else "NOT_SUPPORTED")
 ...
 (out/"INPUT_DATA_RECEIPT.json").write_text(json.dumps({"parent_state_sha256":PSTATE,"parent_cohort_sha256":PCOH,"parent_source":"immutable_phase1_release","year_2026_read":False},indent=2)+"\n")
