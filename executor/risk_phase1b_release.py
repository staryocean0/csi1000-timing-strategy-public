...
def bootstrap_auc_gain(frame: pd.DataFrame, ycol: str, bcol: str, ccol: str) -> dict:
    ...
    return {
        ...
        "point": float(paired(frame, ycol, bcol, ccol)["auroc_gain"]),
    }
...
def evaluate(frame: pd.DataFrame, horizon: int, support: dict) -> tuple[dict, dict | None]:
    if not support["supported_horizon"]:
        return {
            "horizon_minutes": horizon,
            "status": "OUT_OF_SUPPORTED_HORIZON",
    ...
    bootstrap = bootstrap_auc_gain(dev, ycol, bcol, ccol)
    ...
    ordering_gates = {
        ...
        "development_bootstrap_lower_gt_zero": bootstrap["lower"] > 0,
    ...
    freeze = {
        ...
        "repeat_audit": {
            "B": [float(x) for x in audit_fit_b],
            "C": [float(x) for x in audit_fit_c],
        },
    }
...
def run(inputs: Path, out: Path) -> dict:
    ...
    if not state.is_file() or sha256_file(state) != PARENT_STATE_SHA256:
        raise RuntimeError("parent_state_drift")
    if not cohort_path.is_file() or sha256_file(cohort_path) != PARENT_COHORT_SHA256:
        raise RuntimeError("parent_cohort_drift")
    cohort = pd.read_parquet(cohort_path)
    required = {
        "symbol", "trading_day", "year", "normal_within_15m", "normal_within_30m",
        "p_B_15m", "p_C_15m", "p_B_30m", "p_C_30m",
    }
    if not required.issubset(cohort.columns):
        raise RuntimeError("parent_score_columns_missing")
    years = set(int(y) for y in cohort.year.dropna().unique())
    if not years.issubset(set(DEV_YEARS + AUDIT_YEARS)) or any(y >= 2026 for y in years):
        raise RuntimeError("year_boundary_drift")

    supports = {h: support_for_horizon(cohort, h) for h in HORIZONS}
    results = {}
    freezes = {}
    for h in HORIZONS:
    ...
    for h in HORIZONS:
        ...
        metric_rows.append({
            "horizon_minutes": h,
            "status": node["status"],
            "scientific_support": node["scientific_support"],
            "dev_auroc_gain": node.get("raw_development_pooled", {}).get("auroc_gain"),
            "dev_bootstrap_lower": node.get("development_bootstrap", {}).get("lower"),
            "dev_cal_brier_gain": node.get("calibrated_development_pooled", {}).get("brier_gain"),
            "dev_cal_logloss_gain": node.get("calibrated_development_pooled", {}).get("logloss_gain"),
            "audit_auroc_gain": node.get("raw_repeat_audit_pooled", {}).get("auroc_gain"),
            "audit_cal_brier_gain": node.get("calibrated_repeat_audit_pooled", {}).get("brier_gain"),
            "audit_cal_logloss_gain": node.get("calibrated_repeat_audit_pooled", {}).get("logloss_gain"),
        })
    ...
    (out / "INPUT_DATA_RECEIPT.json").write_text(json.dumps({
        ...
        "parent_cohort_sha256": PARENT_COHORT_SHA256,
        "parent_source": "immutable_phase1_release",
        "market_source_read": False,
        "parent_score_refit": False,
        "new_severity_feature_fit": False,
        "years_read": [2021, 2022, 2023, 2024, 2025],
        "year_2026_read": False,
