...
def main():
 ...
 expected={"state_rows.parquet","cohort_rows.parquet","HORIZON_METRICS.csv","SUPPORT_AUDIT.csv","CALIBRATION_FREEZE.json","INPUT_DATA_RECEIPT.json","FRESH_OOS_GATE.json","SUMMARY.json"}
 if {x.name for x in r.iterdir() if x.is_file()}!=expected:raise RuntimeError("result set")
 if sha(r/"state_rows.parquet")!=PSTATE or sha(r/"cohort_rows.parquet")!=PCOH:raise RuntimeError("parent hash")
 x=pd.read_parquet(r/"cohort_rows.parquet");S=json.loads((r/"SUMMARY.json").read_text());F=json.loads((r/"CALIBRATION_FREEZE.json").read_text());G=json.loads((r/"FRESH_OOS_GATE.json").read_text());M=pd.read_csv(r/"HORIZON_METRICS.csv");I=json.loads((r/"INPUT_DATA_RECEIPT.json").read_text())
 if I.get("parent_source")!="immutable_phase1_release" or I.get("year_2026_read") is not False:raise RuntimeError("input lineage")
 eligible=[]
 for h in HS:
