"""Mirror only fixed, already-verified Risk Tool 2.0 text outputs to the private run branch."""
...
import risk_research_broker as risk
...
def main() -> None:
    ...
    if state.get("compute_success") is not True or state.get("cleanup_complete") is not True:
        raise GateError("success_text_mirror_requires_verified_success")
    api = base.require_private_api()
    study = root / "results" / "study"
    for name in TEXT_OUTPUTS:
        path = study / name
        if path.is_symlink() or not path.is_file():
            raise GateError("success_text_output_missing")
        raw = path.read_bytes()
        if len(raw) == 0 or len(raw) > MAX_TEXT_BYTES:
            raise GateError("success_text_output_size_invalid")
        try:
            raw.decode("utf-8")
        except UnicodeDecodeError:
            raise GateError("success_text_output_not_utf8") from None
        target = (
        ...
        api.request(
            target,
            {
                "message": "Record verified Risk Tool text result [skip ci]",
                "branch": state["branch"],
        ...
        if base64.b64decode(returned["content"]) != raw:
            raise GateError("success_text_output_readback_failed")
    print("Verified Risk Tool text outputs mirrored to the private run branch.")
