...
def prepare_inputs(api,root:Path,profile:dict):
    ...
    for name in ("risk_temporal_stability_acceptance.py","risk_temporal_stability_hierarchical_acceptance.py","risk_temporal_stability_hierarchical_v2_acceptance.py"):
        src=HERE/name
        if not src.is_file() or src.is_symlink():raise GateError("2week_acceptance_source_missing")
