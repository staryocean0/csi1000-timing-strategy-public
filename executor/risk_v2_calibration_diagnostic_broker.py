...
HEAD_BRANCH = "research/risk-v2-phase1-calibration-diagnostic-20260914"
SOURCE = {
...
PROFILE = {
    ...
    "manifest_sha256": "53276c7f25861b8a064288f8662c1a391c0c5af75758376df60e98ab180c4234",
...
def require_context() -> None:
    ...
    if (env.get("GITHUB_ACTIONS") != "true" or env.get("GITHUB_REPOSITORY") != rb.PUBLIC_REPO
            or env.get("GITHUB_EVENT_NAME") != "pull_request" or env.get("GITHUB_HEAD_REF") != HEAD_BRANCH):
...
def extract_inputs(archive: Path, work: Path) -> None:
    target = work / "inputs"; target.mkdir()
    seen = set()
    with tarfile.open(archive, "r:gz") as tar:
        for member in tar:
            ...
            if rb.sha(dest) != INPUTS[name]["sha256"]:
...
def main() -> None:
    ...
    parser.add_argument("profile", nargs="?", default=PROFILE_NAME); args = parser.parse_args()
    require_context()
...
def run() -> None:
    try: main()
        ...
        print("Execution stopped: " + str(e), file=sys.stderr); sys.exit(1)
    except Exception:
        print("Execution failed; no private diagnostic content was published.", file=sys.stderr); sys.exit(1)
