...
def qkey(q:float)->str:
    pct=q*100.0
    if float(pct).is_integer(): return "q"+str(int(pct))
...
def load_development(carrier:Path)->pd.DataFrame:
    ...
    x["trading_day"]=x["trading_day"].astype(str); x["symbol"]=x["symbol"].astype(str)
...
def returns_and_boundaries(x:pd.DataFrame):
    z=x.copy(); z["prev_bar_end"]=z.groupby("symbol",sort=False)["bar_end"].shift(1); z["prev_day"]=z.groupby("symbol",sort=False)["trading_day"].shift(1); z["prev_session"]=z.groupby("symbol",sort=False)["session"].shift(1); z["prev_close"]=z.groupby("symbol",sort=False)["close_num"].shift(1)
    z["delta_minutes"]=(z["bar_end"]-z["prev_bar_end"]).dt.total_seconds()/60.0
    valid=z["close_num"].gt(0)&z["prev_close"].gt(0)&z["bar_end"].notna()&z["prev_bar_end"].notna(); same_day=z["trading_day"].eq(z["prev_day"]); same_session=z["session"].eq(z["prev_session"])
    primary=z[valid&same_day&same_session&z["delta_minutes"].eq(15.0)].copy(); primary["log_return"]=np.log(primary["close_num"]/primary["prev_close"]); primary["abs_bps"]=primary["log_return"].abs()*10000.0; primary["clock"]=primary["bar_end"].dt.strftime("%H:%M:%S")
    lunch=z[valid&same_day&z["prev_session"].eq("AM")&z["session"].eq("PM")].copy(); overnight=z[valid&~same_day].copy()
    for name,g in (("lunch",lunch),("overnight",overnight)):
        if len(g): g["log_return"]=np.log(g["close_num"]/g["prev_close"]); g["abs_bps"]=g["log_return"].abs()*10000.0; g["boundary_type"]=name
...
def acf_map(primary:pd.DataFrame):
    ...
    for lag in ACF_LAGS:
        ...
        for _,g in primary.groupby(["symbol","trading_day","session"],sort=False):
            ...
            if len(a)>lag: left.extend(a[:-lag]); right.extend(a[lag:])
        corr=None
        if len(left)>=2 and np.std(left)>0 and np.std(right)>0: corr=rf(np.corrcoef(np.asarray(left),np.asarray(right))[0,1])
        out[str(lag)]={"pair_count":len(left),"corr":corr}
    return out

def run_lengths(primary:pd.DataFrame,threshold:float):
    runs=[]; exceedance_rows=0
    for _,g in primary.groupby(["symbol","trading_day","session"],sort=False):
        flags=g["abs_bps"].to_numpy(dtype=float)>=threshold; exceedance_rows+=int(flags.sum()); cur=0
        for f in flags:
            if f: cur+=1
            elif cur: runs.append(cur); cur=0
        if cur: runs.append(cur)
    if not runs: return {"episodes":0,"exceedance_rows":exceedance_rows,"mean":None,"q50":None,"q90":None,"q95":None,"max":None}
...
def recovery_map(primary:pd.DataFrame,thresholds:dict):
    ...
    for q in RECOVERY_Q:
        ...
        profile={k:{"count":len(vals),"median_ratio":rf(np.quantile(vals,.5)) if vals else None,"q75_ratio":rf(np.quantile(vals,.75)) if vals else None} for k,vals in ratios.items()}
        recovered=len(times); out[key]={"event_count":events,"post_event_ratio_profile":profile,"time_to_below_q75":{"recovered_count":recovered,"unrecovered_count":events-recovered,"recovered_share":rf(recovered/events) if events else None,"median_native_bars":rf(np.quantile(times,.5)) if times else None,"q90_native_bars":rf(np.quantile(times,.9)) if times else None,"max_horizon_native_bars":HORIZON}}
    return out

def recompute_result(inputs:Path):
    carrier=inputs/"15m_offset_5.parquet"; ident=json.loads((inputs/"DATA_IDENTITY.json").read_text()); parent=json.loads((inputs/"PHASE_A_PARENT.json").read_text())
    ...
    close_parse_rate=rf((x["close_num"].gt(0)&x["close_num"].notna()).mean()); primary,boundary=returns_and_boundaries(x)
    if len(primary)==0: raise RuntimeError("no_primary_returns")
    thresholds=qmap(primary["abs_bps"],ABS_Q); years=sorted(int(y) for y in x["year"].dropna().unique())
    trading_days_by_year={str(y):int(x.loc[x["year"].eq(y),"trading_day"].nunique()) for y in DEV_YEARS}; primary_by_year={str(y):int(primary.loc[primary["year"].eq(y)].shape[0]) for y in DEV_YEARS}
    freq_year={}
    for y in DEV_YEARS:
        g=primary[primary["year"].eq(y)]; freq_year[str(y)]={"rows":len(g)}
        for q in TAIL_Q:
            k=qkey(q); freq_year[str(y)][k+"_share"]=rf((g["abs_bps"]>=float(thresholds[k])).mean()) if len(g) else None
    overall={"rows":len(primary)}
    for q in TAIL_Q:
        k=qkey(q); overall[k+"_share"]=rf((primary["abs_bps"]>=float(thresholds[k])).mean())
    by_clock={}; q95=float(thresholds["q95"])
    for clock,g in primary.groupby("clock",sort=True): by_clock[str(clock)]={"rows":len(g),"q95_events":int((g["abs_bps"]>=q95).sum()),"q95_share":rf((g["abs_bps"]>=q95).mean())}
    daily_rv=(primary.groupby(["year","trading_day"],sort=True)["log_return"].apply(lambda s: float(np.sqrt(np.square(s.to_numpy(dtype=float)).sum())*10000.0)).rename("rv_bps").reset_index()); severity_year={}
    for y in DEV_YEARS:
        g=primary[primary["year"].eq(y)]; d=daily_rv[daily_rv["year"].eq(y)]; severity_year[str(y)]={"abs_return_bps":qmap(g["abs_bps"],ABS_Q+[1.0]),"daily_rv_bps":qmap(d["rv_bps"],[.5,.75,.9,.95,.99,1.0])}
...
def main():
    ...
    actual=json.loads((a.results/"NATIVE15_DESCRIPTIVE_RISK_MAP.json").read_text()); expected=recompute_result(a.inputs.resolve())
    if json.dumps(actual,sort_keys=True,separators=(",",":"))!=json.dumps(expected,sort_keys=True,separators=(",",":")): raise RuntimeError("descriptive_map_recompute_mismatch")
    if actual.get("status") not in {"NATIVE15_DESCRIPTIVE_MAP_COMPLETE","NATIVE15_DESCRIPTIVE_MAP_INSUFFICIENT_SUPPORT"}: raise RuntimeError("descriptive_map_status_invalid")
    if any(v is not False for v in actual.get("controls",{}).values()): raise RuntimeError("descriptive_map_control_violation")
    print(json.dumps({"status":"passed","task_id":TASK_ID,"scientific_status":actual["status"]},sort_keys=True))
