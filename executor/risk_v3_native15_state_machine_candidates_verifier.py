...
def candidate_metrics(base:pd.DataFrame, rv:int, bg:int, sigma:float, high:float, recovery:float)->dict:
    z=base.copy(); r=z["log_return"].astype(float)
    ...
    for _,idx in z.groupby("trading_day",sort=False).groups.items():
        ...
        for i in idx:
            ...
            state.at[i]=mode
    z["state"]=state; total=len(z)
    states={s:int(z["state"].eq(s).sum()) for s in ("NORMAL","UNSAFE","RECOVERING")}; shares={s:rf(states[s]/total) if total else None for s in states}
    by_year={}
    for y in DEV_YEARS:
        g=z[z["year"].eq(y)]; n=len(g); counts={s:int(g["state"].eq(s).sum()) for s in ("NORMAL","UNSAFE","RECOVERING")}
    ...
    def capture(thr, year=None):
        g=z if year is None else z[z["year"].eq(year)]; e=g[g["abs_bps"].ge(thr)]
    ...
    for y in DEV_YEARS:
        g=z[z["year"].eq(y)]; current["unsafe_normal_ratio_by_year"][str(y)]=ratio(med(g.loc[g["state"].eq("UNSAFE"),"abs_bps"]),med(g.loc[g["state"].eq("NORMAL"),"abs_bps"]))
    z["next_abs_bps"]=z.groupby(["trading_day","session"],sort=False)["abs_bps"].shift(-1); next_valid=z[z["next_abs_bps"].notna()]
    ...
    for y in DEV_YEARS:
        g=next_valid[next_valid["year"].eq(y)]; nextbar["unsafe_normal_ratio_by_year"][str(y)]=ratio(med(g.loc[g["state"].eq("UNSAFE"),"next_abs_bps"]),med(g.loc[g["state"].eq("NORMAL"),"next_abs_bps"]))
    episodes={"UNSAFE":episode_stats(z["state"],z["trading_day"],"UNSAFE"),"RECOVERING":episode_stats(z["state"],z["trading_day"],"RECOVERING")}
    values={"unsafe_share_overall":shares["UNSAFE"],"normal_share_overall":shares["NORMAL"],"recovering_share_overall":shares["RECOVERING"],"unsafe_rows_each_year":{str(y):by_year[str(y)]["state_rows"]["UNSAFE"] for y in DEV_YEARS},"recovering_rows_each_year":{str(y):by_year[str(y)]["state_rows"]["RECOVERING"] for y in DEV_YEARS},"q95_capture_overall":event_capture["q95_overall"]["unsafe_capture"],"q95_capture_each_year":{str(y):event_capture["q95_by_year"][str(y)]["unsafe_capture"] for y in DEV_YEARS},"q99_capture_overall":event_capture["q99_overall"]["unsafe_capture"],"unsafe_normal_current_severity_ratio_overall":current["unsafe_normal_ratio"],"unsafe_normal_current_severity_ratio_each_year":current["unsafe_normal_ratio_by_year"],"unsafe_normal_next1_persistence_ratio_overall":nextbar["unsafe_normal_ratio"],"unsafe_normal_next1_persistence_ratio_each_year":nextbar["unsafe_normal_ratio_by_year"],"recovering_normal_current_severity_ratio_overall":current["recovering_normal_ratio"]}
    def ge(v,x): return v is not None and v>=x
    gates={"unsafe_share_overall_min":ge(values["unsafe_share_overall"],.02),"unsafe_share_overall_max":values["unsafe_share_overall"] is not None and values["unsafe_share_overall"]<=.20,"normal_share_overall_min":ge(values["normal_share_overall"],.65),"recovering_share_overall_max":values["recovering_share_overall"] is not None and values["recovering_share_overall"]<=.20,"unsafe_rows_each_year_min":all(v>=40 for v in values["unsafe_rows_each_year"].values()),"recovering_rows_each_year_min":all(v>=20 for v in values["recovering_rows_each_year"].values()),"q95_capture_overall_min":ge(values["q95_capture_overall"],.60),"q95_capture_each_year_min":all(ge(v,.35) for v in values["q95_capture_each_year"].values()),"q99_capture_overall_min":ge(values["q99_capture_overall"],.75),"unsafe_normal_current_severity_ratio_overall_min":ge(values["unsafe_normal_current_severity_ratio_overall"],1.50),"unsafe_normal_current_severity_ratio_each_year_min":all(ge(v,1.15) for v in values["unsafe_normal_current_severity_ratio_each_year"].values()),"unsafe_normal_next1_persistence_ratio_overall_min":ge(values["unsafe_normal_next1_persistence_ratio_overall"],1.10),"unsafe_normal_next1_persistence_ratio_each_year_min":all(ge(v,1.00) for v in values["unsafe_normal_next1_persistence_ratio_each_year"].values()),"recovering_normal_current_severity_ratio_overall_min":ge(values["recovering_normal_current_severity_ratio_overall"],1.00),"recovering_current_ratio_below_unsafe_current_ratio":values["recovering_normal_current_severity_ratio_overall"] is not None and values["unsafe_normal_current_severity_ratio_overall"] is not None and values["recovering_normal_current_severity_ratio_overall"]<values["unsafe_normal_current_severity_ratio_overall"]}
    passing=all(gates.values()); min_year_next=min(values["unsafe_normal_next1_persistence_ratio_each_year"].values()) if all(v is not None for v in values["unsafe_normal_next1_persistence_ratio_each_year"].values()) else None
    return {"parameters":{"rv_window":rv,"bg_window":bg,"shock_sigma":sigma,"highvol_ratio":high,"recovery_normal_ratio":recovery},"state_support":{"rows":total,"state_rows":states,"state_share":shares,"by_year":by_year},"event_capture":event_capture,"current_severity":current,"next1_persistence":nextbar,"episodes":episodes,"gate_values":values,"gate_checks":gates,"passing":passing,"ranking_components":{"min_year_next1_ratio":rf(min_year_next) if min_year_next is not None else None,"q95_capture_overall":values["q95_capture_overall"],"current_severity_ratio_overall":values["unsafe_normal_current_severity_ratio_overall"],"unsafe_share_overall":values["unsafe_share_overall"]}}

def sort_key(c):
    r=c["ranking_components"]; missing=-1e99
    return (-float(r["min_year_next1_ratio"] if r["min_year_next1_ratio"] is not None else missing),-float(r["q95_capture_overall"] if r["q95_capture_overall"] is not None else missing),-float(r["current_severity_ratio_overall"] if r["current_severity_ratio_overall"] is not None else missing),float(r["unsafe_share_overall"] if r["unsafe_share_overall"] is not None else 1e99),c["parameters"]["rv_window"],c["parameters"]["bg_window"],c["parameters"]["shock_sigma"],c["parameters"]["highvol_ratio"],c["parameters"]["recovery_normal_ratio"])

def build_result(inputs:Path):
    carrier=inputs/"15m_offset_5.parquet"; ident=json.loads((inputs/"DATA_IDENTITY.json").read_text()); parent=json.loads((inputs/"PHASE_B_PARENT.json").read_text())
    ...
    passing=sorted([c for c in candidates if c["passing"]],key=sort_key); shortlist=passing[:SHORTLIST_MAX]
...
def main():
    p=argparse.ArgumentParser(); p.add_argument("--inputs",type=Path,required=True); p.add_argument("--results",type=Path,required=True); a=p.parse_args()
    actual=json.loads((a.results/"NATIVE15_STATE_MACHINE_CANDIDATE_MAP.json").read_text()); actual_summary=json.loads((a.results/"NATIVE15_PHASE_C1_SUMMARY.json").read_text())
    expected,expected_summary=build_result(a.inputs.resolve()); canon=lambda v: json.dumps(v,sort_keys=True,separators=(",",":"))
    if canon(actual)!=canon(expected): raise RuntimeError("native15_c1_candidate_map_recompute_mismatch")
    if canon(actual_summary)!=canon(expected_summary): raise RuntimeError("native15_c1_summary_recompute_mismatch")
    if actual.get("status") not in {"NATIVE15_STATE_MACHINE_CANDIDATE_MAP_COMPLETE","NATIVE15_STATE_MACHINE_CANDIDATE_MAP_INSUFFICIENT"}: raise RuntimeError("native15_c1_status_invalid")
    if any(v is not False for v in actual.get("controls",{}).values()): raise RuntimeError("native15_c1_control_violation")
    if actual.get("candidate_count")!=576: raise RuntimeError("native15_c1_candidate_count_invalid")
    if actual.get("shortlist_count",0)>12: raise RuntimeError("native15_c1_shortlist_too_large")
    if actual.get("next_phase_authorized") is True and actual.get("passing_count",0)<3: raise RuntimeError("native15_c1_authorization_without_support")
    print(json.dumps({"status":"passed","task_id":TASK_ID,"scientific_status":actual["status"],"passing_count":actual["passing_count"]},sort_keys=True))
