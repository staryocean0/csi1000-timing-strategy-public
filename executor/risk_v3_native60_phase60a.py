...
def build(inputs:Path,out:Path)->dict:
    ...
    meta=ident.get("carrier") or {};carrier=inputs/"1m_official.parquet"
    ...
    frame=dataset.to_table(columns=required,filter=filt).to_pandas(ignore_metadata=True)
    parsed=pd.to_datetime(frame[tf].astype(str).str.slice(0,19),errors="coerce")
    parse_rate=float(parsed.notna().mean()) if len(parsed) else 0.0
    dev=pd.DataFrame({"bar_time":parsed,"trading_day":frame["trading_day"].astype(str),"symbol":frame["symbol"].astype(str),"open":pd.to_numeric(frame["open"],errors="coerce"),"high":pd.to_numeric(frame["high"],errors="coerce"),"low":pd.to_numeric(frame["low"],errors="coerce"),"close":pd.to_numeric(frame["close"],errors="coerce")})
    dev=dev[dev["bar_time"].notna()].sort_values(["trading_day","bar_time"],kind="stable").reset_index(drop=True)
    dev["year"]=pd.to_numeric(dev["trading_day"].str.slice(0,4),errors="coerce").astype("Int64")
    ...
    total_days=len(day_minutes);shares={c:(convention_counts[c]/total_days if total_days else 0.0) for c in convention_counts}
    ...
    if convention:
        ...
        for d,g in dev.groupby("trading_day",sort=True):
            if len(g)==240 and set(map(int,g["minute_of_day"]))==exp and not g.duplicated(["bar_time"]).any():accepted_days.append(d)
    accepted=set(accepted_days);complete_share=(len(accepted)/total_days) if total_days else 0.0
    by_year={}
    for y in YEARS:
        all_days=sorted(dev.loc[dev["year"].eq(y),"trading_day"].unique().tolist());acc=[d for d in all_days if d in accepted]
    ...
    rows=[];bucket_counts=Counter()
    if convention:
        for d in accepted_days:
            g=dev[dev["trading_day"].eq(d)].copy();g["slot"]=[slot_for_minute(int(m),convention) for m in g["minute_of_day"]]
            for slot,start,end,start_clock,end_clock in SLOTS:
                b=g[g["slot"].eq(slot)].sort_values("bar_time",kind="stable");bucket_counts[(slot,len(b))]+=1
                ...
                rows.append({"trading_day":d,"symbol":SYMBOL,"slot":slot,"bar_start":f"{d} {start_clock}","bar_end":f"{d} {end_clock}","timestamp_convention":convention,"minute_count":60,"open":o,"high":h,"low":l,"close":c,"body_abs_log":abs(math.log(c/o)),"range_log":math.log(h/l),"intrabar_rv_1m":float(math.sqrt(float(np.square(minute_lr).sum())))})
    ...
    technical={"source_identity_exact":carrier.stat().st_size==SOURCE_BYTES and sha(carrier)==SOURCE_SHA256,"timestamp_parse_rate_ge_09999":parse_rate>=0.9999,"duplicate_symbol_timestamp_rows_eq_0":dup==0,"unique_timestamp_convention":convention is not None,"development_trading_days_each_year_ge_200":all(by_year[str(y)]["accepted_complete_days"]>=200 for y in YEARS),"complete_day_share_ge_095":complete_share>=0.95,"ohlc_invalid_rows_eq_0":invalid_ohlc==0,"canonical_bucket_minute_count_eq_60":bool(len(canon)>0 and canon["minute_count"].eq(60).all()),"canonical_bars_per_complete_day_eq_4":bool(len(canon)>0 and (bars_per_day==4).all())}
    valid=all(technical.values())
    out.mkdir(parents=True,exist_ok=False);canonical_path=out/"NATIVE60_CANONICAL_60M.parquet"
    ...
    result={"schema_id":SCHEMA_ID,"task_id":TASK_ID,"profile":PROFILE,"prereg_sha256":PREREG_SHA256,"status":"NATIVE60_CANONICAL_CARRIER_VALID" if valid else "NATIVE60_CANONICAL_CARRIER_NOT_READY","source":{"archive_path":meta.get("archive_path"),"bytes":SOURCE_BYTES,"sha256":SOURCE_SHA256,"physical_rows":int(pf.metadata.num_rows)},"schema":{"time_field":tf,"physical_fields":[{"name":f.name,"type":str(f.type)} for f in pf.schema_arrow]},"timestamp_semantics":{"selected_convention":convention,"compatible_conventions":compatible,"exact_complete_day_counts":convention_counts,"exact_complete_day_shares":{k:rf(v) for k,v in shares.items()}},"development":{"years":list(YEARS),"symbol":SYMBOL,"parsed_rows":int(len(dev)),"trading_days":total_days,"accepted_complete_days":len(accepted),"complete_day_share":rf(complete_share),"by_year":by_year,"duplicate_symbol_timestamp_rows":dup,"ohlc_invalid_rows":invalid_ohlc},"canonical":{"geometry":[x[0] for x in SLOTS],"rows":int(len(canon)),"trading_days":int(canon.trading_day.nunique()) if len(canon) else 0,"bars_per_day_unique":sorted(map(int,bars_per_day.unique().tolist())) if len(bars_per_day) else [],"bucket_member_count_summary":{f"{slot}:{n}":int(v) for (slot,n),v in sorted(bucket_counts.items())},"private_parquet":canonical_identity},"technical_acceptance":technical,"controls":{"use_2024_2025_market_values":False,"use_2026_market_values":False,"risk_threshold_search":False,"state_machine_installation":False,"model_fit":False,"calibration":False,"pnl":False,"strategy_routing":False,"position_sizing":False,"production_authority":False,"new_training":False},"next_phase_authorized":bool(valid),"next_phase":"native60_phase60b_volatility_continuity_map_preregister_separately" if valid else None}
