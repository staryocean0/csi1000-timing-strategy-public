...
def independently_recompute(inputs:Path):
    ...
    pf=pq.ParquetFile(carrier);names=list(pf.schema_arrow.names)
    ...
    raw=pf.read(columns=required,use_pandas_metadata=False).to_pandas(ignore_metadata=True)
    parsed=pd.to_datetime(raw[tf].astype(str).str.slice(0,19),errors="coerce")
    parse_rate=float(parsed.notna().mean()) if len(parsed) else 0.0
    z=pd.DataFrame({
        "bar_time":parsed,
        "trading_day":raw["trading_day"].astype(str),
    ...
    dev=z[z["symbol"].eq(SYMBOL)&z["year"].isin(YEARS)&z["bar_time"].notna()].copy()
    dev=dev.sort_values(["trading_day","bar_time"],kind="stable").reset_index(drop=True)
    dev["minute_of_day"]=dev["bar_time"].dt.hour*60+dev["bar_time"].dt.minute
    dup=int(dev.duplicated(["symbol","bar_time"]).sum())
    good_num=dev[["open","high","low","close"]].gt(0).all(axis=1)
    good_order=(dev["high"]>=dev[["open","close"]].max(axis=1))&(dev["low"]<=dev[["open","close"]].min(axis=1))&(dev["high"]>=dev["low"])
    invalid_ohlc=int((~(good_num&good_order)).sum())

    daysets={d:set(map(int,g["minute_of_day"].tolist())) for d,g in dev.groupby("trading_day",sort=True)}
    total_days=len(daysets)
    exact={c:sum(1 for s in daysets.values() if s==expected(c)) for c in ("end_labeled","start_labeled")}
    shares={c:(exact[c]/total_days if total_days else 0.0) for c in exact}
    compatible=[]
    ...
    aset=set(accepted);complete_share=(len(accepted)/total_days) if total_days else 0.0
    by_year={}
    for y in YEARS:
        alld=sorted(dev.loc[dev["year"].eq(y),"trading_day"].unique().tolist());acc=[d for d in alld if d in aset]
        by_year[str(y)]={"trading_days":len(alld),"accepted_complete_days":len(acc),"complete_day_share":(len(acc)/len(alld) if alld else 0.0)}
    ...
    if convention:
        for d in accepted:
            ...
            for slot,start,end,start_clock,end_clock in SLOTS:
                ...
                lr=np.log(b["close"].astype(float).to_numpy()/b["open"].astype(float).to_numpy())
    ...
    if len(canon):
        order={"AM1":1,"AM2":2,"PM1":3,"PM2":4}
        canon["_ord"]=canon["slot"].map(order);canon=canon.sort_values(["trading_day","_ord"],kind="stable").drop(columns="_ord").reset_index(drop=True)
    ...
    technical={
        ...
        "unique_timestamp_convention":convention is not None,
        "development_trading_days_each_year_ge_200":all(by_year[str(y)]["accepted_complete_days"]>=200 for y in YEARS),
        "complete_day_share_ge_095":complete_share>=0.95,
        "ohlc_invalid_rows_eq_0":invalid_ohlc==0,
        "canonical_bucket_minute_count_eq_60":bool(len(canon)>0 and canon["minute_count"].eq(60).all()),
        "canonical_bars_per_complete_day_eq_4":bool(len(canon)>0 and (bpd==4).all()),
    }
...
def verify(inputs:Path,results:Path):
    audit=load(results/"NATIVE60_PHASE60A_AUDIT.json")
    produced_path=results/"NATIVE60_CANONICAL_60M.parquet"
    ...
    canonical_identity={"bytes":produced_path.stat().st_size,"sha256":sha(produced_path),"rows":int(len(expected_df))}
    expected_audit={
        "schema_id":SCHEMA_ID,"task_id":TASK_ID,"profile":PROFILE,"prereg_sha256":PREREG_SHA256,
        "status":"NATIVE60_CANONICAL_CARRIER_VALID" if all(technical.values()) else "NATIVE60_CANONICAL_CARRIER_NOT_READY",
        ...
        "timestamp_semantics":{"selected_convention":convention,"compatible_conventions":compatible,"exact_complete_day_counts":exact,"exact_complete_day_shares":{k:rf(v) for k,v in shares.items()}},
        "development":{"years":list(YEARS),"symbol":SYMBOL,"parsed_rows":int(len(dev)),"trading_days":len(set(dev["trading_day"])),"accepted_complete_days":len(accepted),"complete_day_share":rf(complete_share),"by_year":by_year,"duplicate_symbol_timestamp_rows":dup,"ohlc_invalid_rows":invalid_ohlc},
        "canonical":{"geometry":[x[0] for x in SLOTS],"rows":int(len(expected_df)),"trading_days":int(expected_df.trading_day.nunique()) if len(expected_df) else 0,"bars_per_day_unique":sorted(map(int,bpd.unique().tolist())) if len(bpd) else [],"bucket_member_count_summary":{f"{slot}:{n}":int(v) for (slot,n),v in sorted(counts.items())},"private_parquet":canonical_identity},
        ...
        "next_phase_authorized":bool(all(technical.values())),
        "next_phase":"native60_phase60b_volatility_continuity_map_preregister_separately" if all(technical.values()) else None,
    }
