...
def verify(inputs:Path,results:Path):
    path=results/"NATIVE60_PHASE60C1_STATE_SCORE.json"
    if not path.is_file() or path.is_symlink():raise RuntimeError("verifier_result_missing")
    got=json.loads(path.read_text())
    expected=build_result(inputs)
    if got!=expected:raise RuntimeError("verifier_exact_compare_failed")
    return {"status":"passed","scientific_status":got["status"],"eligible_count":got["eligible_count"],"selected":None if got["selected_score"] is None else got["selected_score"]["candidate_id"]}
...
def main():
    ...
    print(json.dumps(verify(a.inputs.resolve(),a.results.resolve()),sort_keys=True))
