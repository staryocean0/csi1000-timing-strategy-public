...
def weekly_buckets(temporal,base,cohort,freeze,profile,market_days):
    expected=temporal.labels_for_days(market_days)["weekly"]
    ordered=sorted(expected.items())
    gate=profile["levels"]["weekly"]["support"]
    rows=[]
    for h in HORIZONS:
        yc=f"normal_within_{h}m"
        z=cohort[cohort[yc].notna()].copy()
        z["p_B"]=base.predict(freeze["models"][str(h)]["B"],z)
        z["p_C"]=base.predict(freeze["models"][str(h)]["C"],z)
        ...
        for i,(label,role) in enumerate(ordered):
            ...
            positive=int(y.sum());negative=int(len(y)-positive)
            supported=bool(len(q)>=gate["rows_min"] and positive>=gate["positive_min"] and negative>=gate["negative_min"])
            auc_b=auc_c=gain=delta_auc=delta_pos=delta_neg=delta_gap=float("nan")
            event_rate=float(y.mean()) if len(y) else float("nan")
            if supported:
                auc_b=temporal.auc(y,q.p_B);auc_c=temporal.auc(y,q.p_C);gain=float(auc_c-auc_b)
                delta=(q.p_C-q.p_B).to_numpy(float);delta_auc=temporal.auc(y,delta)
                delta_pos=float(delta[y==1].mean());delta_neg=float(delta[y==0].mean());delta_gap=float(delta_pos-delta_neg)
...
def run(inputs:Path,out:Path):
    temporal=load_module(inputs/"temporal_base.py","weekly_order_temporal")
    base=temporal.load_base(inputs);freeze,_cal,frames,receipt=temporal.load_inputs(inputs)
    frames,excluded=temporal.filter_complete_days(base,frames);states=base.build_state_rows(frames);cohort=base.build_primary_cohort(states)
    ...
    market_days=pd.concat([pd.to_datetime(f["trading_day"],errors="coerce") for f in frames.values()],ignore_index=True)
    buckets=weekly_buckets(temporal,base,cohort,freeze,profile,market_days);streak_table=streaks(buckets);summary=summarize(buckets,streak_table)
    out.mkdir(parents=True,exist_ok=False);buckets.to_csv(out/"WEEKLY_ORDERING_BUCKETS.csv",index=False);streak_table.to_csv(out/"NEGATIVE_STREAKS.csv",index=False)
