"""Mirror verified weekly-window feasibility text outputs privately."""
...
FILES = ("SUMMARY.json", "WINDOW_FEASIBILITY.csv", "YEARLY_WINDOW_FEASIBILITY.csv", "SELECTED_WINDOW_BUCKETS.csv", "INPUT_RECEIPT.json")
...
def main():
    ...
    if state.get("profile_name") != PROFILE or state.get("compute_success") is not True or state.get("cleanup_complete") is not True:
        raise GateError("weekly_window_feasibility_state_not_mirrorable")
    ...
    for name in FILES:
        ...
        if path.is_symlink() or not path.is_file() or not 0 < path.stat().st_size <= 512 * 1024:
            raise GateError("weekly_window_feasibility_mirror_file_invalid")
        raw = path.read_bytes()
        try: raw.decode("utf-8")
        except UnicodeDecodeError: raise GateError("weekly_window_feasibility_mirror_file_not_utf8") from None
        target = f"repos/{base.PRIVATE_REPO}/contents/research/public-runs/{state['run_id']}-results/{name}"
        api.request(target, {"message":"Record verified weekly window feasibility [skip ci]","branch":state["branch"],"content":base64.b64encode(raw).decode("ascii")}, method="PUT")
        got = api.request(target + "?ref=" + base.urllib.parse.quote(state["branch"], safe=""))
        if base64.b64decode(got["content"]) != raw: raise GateError("weekly_window_feasibility_mirror_readback_failed")
    print("Verified weekly window feasibility mirrored privately.")
