"""Reviewed broker for Two-Wave V0800-B1 strict-continuity diagnostic."""
...
PROFILE_NAME = "two-wave-v0800-b1-strict-continuity-v1"
...
PROTOCOL = HERE.parent / "docs" / "research" / "TWO_WAVE_V0800_B1_STRICT_CONTINUITY_PROTOCOL.json"
SCRIPT_NAMES = (
    "two_wave_v0800_semantics.py",
    "two_wave_v0800_scale_map.py",
    "two_wave_v0800_b1_strict_continuity.py",
    "two_wave_v0800_b1_strict_continuity_verifier.py",
)
...
def profile() -> dict:
    if not PROTOCOL.is_file():
        raise GateError("two_wave_v0800_b1_protocol_missing")
    return {
        "private_ref": PRIVATE_REF,
        "manifest_sha256": sha256(PROTOCOL),
        "command": ["two_wave_b1/two_wave_v0800_b1_strict_continuity.py", "--inputs", "/work/inputs", "--out", "/results/study"],
        "verify_command": ["two_wave_b1/two_wave_v0800_b1_strict_continuity_verifier.py", "--inputs", "/work/inputs", "--results", "/results/study"],
...
def require_context() -> None:
    ...
    if (
        ...
        raise GateError("not_approved_two_wave_v0800_b1_context")
...
def download_public_data(target: Path) -> None:
    ...
    with urllib.request.urlopen(request, timeout=90) as response:
        ...
        if final.scheme != "https" or final.hostname != "raw.githubusercontent.com":
            raise GateError("two_wave_v0800_b1_public_redirect_rejected")
        ...
        with target.open("xb") as output:
            while True:
                ...
                total += len(chunk)
                if total > DATA_BYTES:
                    raise GateError("two_wave_v0800_b1_public_data_oversize")
                digest.update(chunk)
                output.write(chunk)
    if total != DATA_BYTES or digest.hexdigest() != DATA_SHA256:
        raise GateError("two_wave_v0800_b1_public_data_identity_failed")
...
def prepare_inputs(api, root: Path, fixed_profile: dict) -> Path:
    ...
    inputs = work / "inputs"
    inputs.mkdir()
    scripts = work / "two_wave_b1"
    scripts.mkdir()
    for name in SCRIPT_NAMES:
        source = HERE / name
        if not source.is_file() or source.is_symlink():
            raise GateError("two_wave_v0800_b1_public_source_missing")
    ...
    if sha256(protocol_target) != profile()["manifest_sha256"]:
        raise GateError("two_wave_v0800_b1_protocol_identity_failed")
...
def main() -> None:
    ...
    if args.profile != PROFILE_NAME:
        raise GateError("unknown_two_wave_v0800_b1_profile")
    fixed = profile()
...
def run() -> None:
    try:
        ...
        print("Execution failed; no private Two-Wave B1 diagnostic content was exposed publicly.", file=sys.stderr)
        raise SystemExit(1)
