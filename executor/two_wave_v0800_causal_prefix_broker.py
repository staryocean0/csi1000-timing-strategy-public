...
rb.COMPUTE_HOST_TIMEOUT_SECONDS = 900
rb.VALIDATE_HOST_TIMEOUT_SECONDS = 900

PROFILE_NAME = "two-wave-v0800-causal-prefix-v1"
...
PROTOCOL = HERE.parent / "docs" / "research" / "TWO_WAVE_V0800_C_CAUSAL_PREFIX_PROTOCOL.json"
SCRIPT_NAMES = (
    "two_wave_v0800_semantics.py",
    "two_wave_v0800_scale_map.py",
    "two_wave_v0800_causal_prefix.py",
    "two_wave_v0800_causal_prefix_verifier.py",
)
...
def profile() -> dict:
    if not PROTOCOL.is_file():
        raise GateError("two_wave_v0800_causal_protocol_missing")
    return {
        "private_ref": PRIVATE_REF,
        "manifest_sha256": sha256(PROTOCOL),
        "command": ["two_wave/two_wave_v0800_causal_prefix.py", "--inputs", "/work/inputs", "--out", "/results/study"],
        "verify_command": ["two_wave/two_wave_v0800_causal_prefix_verifier.py", "--inputs", "/work/inputs", "--results", "/results/study"],
        "command_timeout_seconds": 840,
        "verification_timeout_seconds": 840,
        "new_training": False,
...
def require_context() -> None:
    ...
    if (
        ...
        raise GateError("not_approved_two_wave_v0800_causal_context")
...
def download_public_data(target: Path) -> None:
    ...
    with urllib.request.urlopen(req, timeout=90) as response:
        ...
        if final.scheme != "https" or final.hostname != "raw.githubusercontent.com":
            raise GateError("two_wave_v0800_causal_public_redirect_rejected")
        ...
        with target.open("xb") as output:
            while True:
                ...
                total += len(chunk)
                if total > DATA_BYTES:
                    raise GateError("two_wave_v0800_causal_public_data_oversize")
                digest.update(chunk)
                output.write(chunk)
    if total != DATA_BYTES or digest.hexdigest() != DATA_SHA256:
        raise GateError("two_wave_v0800_causal_public_data_identity_failed")
...
def prepare_inputs(api, root: Path, fixed_profile: dict) -> Path:
    ...
    inputs = work / "inputs"
    inputs.mkdir()
    scripts = work / "two_wave"
    scripts.mkdir()
    for name in SCRIPT_NAMES:
        source = HERE / name
        if not source.is_file() or source.is_symlink():
            raise GateError("two_wave_v0800_causal_public_source_missing")
        shutil.copy2(source, scripts / name)
    protocol_target = scripts / "PROTOCOL.json"
    shutil.copy2(PROTOCOL, protocol_target)
    if sha256(protocol_target) != profile()["manifest_sha256"]:
        raise GateError("two_wave_v0800_causal_protocol_identity_failed")
    download_public_data(inputs / DATA_FILE_NAME)
    return work


DATA_FILE_NAME = "5m_offset_0.parquet"
...
def main() -> None:
    ...
    if args.profile != PROFILE_NAME:
        raise GateError("unknown_two_wave_v0800_causal_profile")
    fixed = profile()
...
def run() -> None:
    try:
        ...
        print("Execution failed; no private Two-Wave causal replay content was exposed publicly.", file=sys.stderr)
        raise SystemExit(1)
