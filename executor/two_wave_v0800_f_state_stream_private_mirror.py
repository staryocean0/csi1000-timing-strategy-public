...
def validate_summary(raw: bytes) -> None:
    ...
    if int(value.get("strict_event_count", -1)) != 2358 or int(value.get("eligible_event_count", -1)) != 924:
        raise GateError("two_wave_v0800_f_parent_universe_mismatch")
    if value.get("operational_rho_symbolic") != "sqrt(2)" or float(value.get("tau", -1)) != 0.20 or float(value.get("kappa", -1)) != 2.00:
        raise GateError("two_wave_v0800_f_parameter_freeze_mismatch")
    if value.get("primary_stream") != "all_strict_pair_confirmation_events" or value.get("epoch_boundary_breaks_sequence") is not True:
        raise GateError("two_wave_v0800_f_stream_contract_mismatch")
    if value.get("bar_time_carry_forward") is not False or value.get("between_event_state") != "undefined":
        raise GateError("two_wave_v0800_f_bar_time_semantics_violation")
    for key in (
        "automatic_acceptance", "parameter_search_used", "direction_acceptance", "state_publication_authority",
    ...
    if value.get("post_run_adjudication_required") is not True:
        raise GateError("two_wave_v0800_f_adjudication_gate_missing")
...
def main() -> None:
    ...
    for name in TEXT_OUTPUTS:
        ...
        api.request(target, {"message": "Record verified Two-Wave V0800-F aggregate result [skip ci]", "branch": state["branch"], "content": base64.b64encode(raw).decode("ascii")}, method="PUT")
