"""Reviewed broker for Two-Wave V0800-G bar-time carrier semantics audit.

This file stages only the frozen public Two-Wave sources and the pinned
Development carrier.  It is intentionally not wired into the shared workflow
or controller in the source-only review phase.
"""
...
PROFILE_NAME = "two-wave-v0800-g-bar-time-carrier-semantics-v1"
...
PROTOCOL = HERE.parent / "docs" / "research" / "TWO_WAVE_V0800_G_BAR_TIME_CARRIER_SEMANTICS_PROTOCOL.json"
SCRIPT_NAMES = (
    "two_wave_v0800_semantics.py",
    "two_wave_v0800_scale_map.py",
    "two_wave_v0800_c_prefix_replay.py",
    "two_wave_v0800_g_bar_time_carrier.py",
    "two_wave_v0800_g_bar_time_carrier_verifier.py",
)
...
def profile() -> dict:
    if not PROTOCOL.is_file() or PROTOCOL.is_symlink():
        raise GateError("two_wave_v0800_g_protocol_missing")
    return {
        "private_ref": PRIVATE_REF,
        "manifest_sha256": sha256(PROTOCOL),
        "command": [
            "two_wave_g/two_wave_v0800_g_bar_time_carrier.py",
            "--inputs", "/work/inputs",
            "--out", "/results/study",
        ],
        "verify_command": [
            "two_wave_g/two_wave_v0800_g_bar_time_carrier_verifier.py",
            "--inputs", "/work/inputs",
...
def require_context() -> None:
    ...
    if (
        ...
        raise GateError("not_approved_two_wave_v0800_g_context")
...
def download_public_data(target: Path) -> None:
    ...
    with urllib.request.urlopen(request, timeout=90) as response:
        ...
        if final.scheme != "https" or final.hostname != "raw.githubusercontent.com":
            raise GateError("two_wave_v0800_g_public_redirect_rejected")
        ...
        with target.open("xb") as output:
            while True:
                ...
                total += len(chunk)
                if total > DATA_BYTES:
                    raise GateError("two_wave_v0800_g_public_data_oversize")
                digest.update(chunk)
                output.write(chunk)
    if total != DATA_BYTES or digest.hexdigest() != DATA_SHA256:
        raise GateError("two_wave_v0800_g_public_data_identity_failed")
...
def prepare_inputs(api, root: Path, fixed_profile: dict) -> Path:
    ...
    inputs = work / "inputs"
    inputs.mkdir()
    scripts = work / "two_wave_g"
    scripts.mkdir()
    for name in SCRIPT_NAMES:
        source = HERE / name
        if not source.is_file() or source.is_symlink():
            raise GateError("two_wave_v0800_g_public_source_missing")
        shutil.copy2(source, scripts / name)
    staged_protocol = scripts / "PROTOCOL.json"
    shutil.copy2(PROTOCOL, staged_protocol)
    if sha256(staged_protocol) != profile()["manifest_sha256"]:
        raise GateError("two_wave_v0800_g_protocol_identity_failed")
...
def main() -> None:
    ...
    if args.profile != PROFILE_NAME:
        raise GateError("unknown_two_wave_v0800_g_profile")
    fixed = profile()
...
def run() -> None:
    try:
        ...
        print("Execution failed; no private Two-Wave G carrier content was exposed publicly.", file=sys.stderr)
        raise SystemExit(1)
