"""Reviewed broker for Two-Wave V0800-B duration/same-scale map."""
...
PROFILE_NAME = "two-wave-v0800-duration-scale-map-v1"
...
def require_context() -> None:
    ...
    if (
        ...
        raise GateError("not_approved_two_wave_v0800_context")
...
def download_public_data(target: Path) -> None:
    ...
    with urllib.request.urlopen(req, timeout=90) as response:
        ...
        if final.scheme != "https" or final.hostname != "raw.githubusercontent.com":
            raise GateError("two_wave_v0800_public_redirect_rejected")
        ...
        with target.open("xb") as output:
            while True:
                ...
                total += len(chunk)
                if total > DATA_BYTES:
                    raise GateError("two_wave_v0800_public_data_oversize")
                digest.update(chunk)
                output.write(chunk)
    if total != DATA_BYTES or digest.hexdigest() != DATA_SHA256:
        raise GateError("two_wave_v0800_public_data_identity_failed")
...
def main() -> None:
    ...
    if args.profile != PROFILE_NAME:
        raise GateError("unknown_two_wave_v0800_profile")
    fixed = profile()
...
def run() -> None:
    try:
        ...
        print("Execution failed; no private Two-Wave diagnostic content was exposed publicly.", file=sys.stderr)
        raise SystemExit(1)
