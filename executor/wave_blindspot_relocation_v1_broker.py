"""Fixed blindspot relocation broker. Only the standard workflow_dispatch may call this.
...
def load_profile():
    ...
    if set(m)!={'schema_id','profile','private_ref','sources','new_training','production_authority'} or m['schema_id']!='csi1000.blindspot_relocation_execution@1.0' or m['profile']!=PROFILE or m['private_ref']!=PRIVATE_REF or set(m['sources'])!=set(SOURCES):
...
def publish(rb,profile):
    ...
    release=api.request(f'repos/{rb.PRIVATE_REPO}/releases',dict(tag_name=tag,target_commitish=PRIVATE_REF,draft=True,prerelease=True,name='Blindspot relocation v1 '+state['run_id'],body='Pending bounded upload verification.'),method='POST')
    ...
    api.request(target,dict(message='Record bounded blindspot-relocation result [skip ci]',branch=state['branch'],content=base64.b64encode(payload).decode()),method='PUT')
    ...
    for name,raw in mirrors.items():
        ...
        api.request(mirror_target,dict(message='Record fixed blindspot relocation readback [skip ci]',branch=state['branch'],content=base64.b64encode(raw).decode()),method='PUT')
    ...
    print('Blindspot relocation v1 private archive and receipt verified; no production authority.')
...
def main():
    ...
    def checked_state():
        ...
        if state.get('blindspot_relocation_manifest_sha256')!=digest(MANIFEST):raise rb.GateError('manifest_changed_between_phases')
        if state.get('profile_name')!=PROFILE:raise rb.GateError('wrong_run_profile')
        return state,root
    def prepare_inputs(api,root,fixed):
        del api,fixed
        work=root/'work';work.mkdir();inputs=work/'inputs';inputs.mkdir();scripts=work/'blindspot_relocation';scripts.mkdir()
    ...
    def docker_command(work,results,path,validator=False):
        ...
        command[index]='/work/blindspot_relocation/wave_blindspot_relocation_v1_runtime.py'
        return command
    ...
    if a.phase=='prepare':
        rb.prepare(PROFILE,profile);state,root=original_state();state['blindspot_relocation_manifest_sha256']=digest(MANIFEST);rb.write_state(state)
...
if __name__=='__main__':
    try:main()
        ...
        print('Blindspot relocation v1 stopped; consult bounded private evidence.',file=__import__('sys').stderr)
        raise SystemExit(1)
