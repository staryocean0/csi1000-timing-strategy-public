...
PROFILE='two-wave-c3-router-v1'
PRIVATE_REF='7688ba57206dd29fbef88d8e57475255718471fe'
MANIFEST=HERE.parent/'docs/research/TWO_WAVE_C3_ROUTER_EXECUTION_MANIFEST_V1.json'
SOURCES=('wave_c3_router_v1.py','wave_c3_router_v1_entry.py','wave_c3_router_v1_verifier.py','wave_c3_router_v1_runtime.py','wave_multiscale_dual_gates_v2.py','wave_dual_gate_hierarchy_v1.py','wave_dual_gate_probe_v1.py','two_wave_v0800_scale_map.py','two_wave_v0800_semantics.py')
...
def load_profile():
    ...
    if set(m)!={'schema_id','profile','private_ref','sources','new_training','production_authority'} or m['schema_id']!='csi1000.c3_router_execution@1.0' or m['profile']!=PROFILE or m['private_ref']!=PRIVATE_REF or set(m['sources'])!=set(SOURCES):
...
def publish(rb,profile):
    ...
    release=api.request(f'repos/{rb.PRIVATE_REPO}/releases',dict(tag_name=tag,target_commitish=PRIVATE_REF,draft=True,prerelease=True,name='C3 router v1 '+state['run_id'],body='Pending bounded upload verification.'),method='POST')
    ...
    print('C3 router v1 private archive and receipt verified; no production authority.')
...
def main():
    ...
    def checked_state():
        ...
        if state.get('c3_router_manifest_sha256')!=digest(MANIFEST):raise rb.GateError('manifest_changed_between_phases')
        if state.get('profile_name')!=PROFILE:raise rb.GateError('wrong_run_profile')
        return state,root
    def prepare_inputs(api,root,fixed):
        del api,fixed
        work=root/'work';work.mkdir();inputs=work/'inputs';inputs.mkdir();scripts=work/'c3_router';scripts.mkdir()
    ...
    if a.phase=='prepare':
        rb.prepare(PROFILE,profile);state,root=original_state();state['c3_router_manifest_sha256']=digest(MANIFEST);rb.write_state(state)
...
if __name__=='__main__':
    try:main()
        ...
        print('C3 router v1 stopped; consult bounded private evidence.',file=__import__('sys').stderr)
        raise SystemExit(1)
