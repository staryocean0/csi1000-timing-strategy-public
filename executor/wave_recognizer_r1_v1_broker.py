"""Fixed recognizer R1 broker. Only the standard workflow_dispatch may call this.
...
def load_profile():
    ...
    if set(m)!={'schema_id','profile','private_ref','sources','new_training','production_authority'} or m['schema_id']!='csi1000.recognizer_r1_execution@1.0' or m['profile']!=PROFILE or m['private_ref']!=PRIVATE_REF or set(m['sources'])!=set(SOURCES):
...
def publish(rb,profile):
    ...
    release=api.request(f'repos/{rb.PRIVATE_REPO}/releases',dict(tag_name=tag,target_commitish=PRIVATE_REF,draft=True,prerelease=True,name='Recognizer R1 '+state['run_id'],body='Pending bounded upload verification.'),method='POST')
    ...
    api.request(target,dict(message='Record bounded recognizer-r1 result [skip ci]',branch=state['branch'],content=base64.b64encode(payload).decode()),method='PUT')
    ...
    if state.get('compute_success'):
        ...
        visual_root=results/'study'/'visuals'
        index_file=visual_root/'index.json'
        if index_file.is_symlink() or index_file.stat().st_size>2_000_000:raise rb.GateError('visual_index_bound')
        index=json.loads(index_file.read_text())
        mirrors['visual_index.json']=index_file.read_bytes()
        # Bounded private-only readback; full pack remains in verified archive.
        for item in index['panels'][:24]:
            name=item['file']
            if not __import__('re').fullmatch(r'case-[0-9a-f]{16}-[0-9]+\.svg',name):raise rb.GateError('visual_filename')
            image=visual_root/name
    ...
    for name,raw in mirrors.items():
        ...
        api.request(mirror_target,dict(message='Record fixed recognizer R1 readback [skip ci]',branch=state['branch'],content=base64.b64encode(raw).decode()),method='PUT')
    ...
    print('Recognizer R1 private archive and receipt verified; no production authority.')
...
def main():
    ...
    def checked_state():
        ...
        if state.get('recognizer_r1_manifest_sha256')!=digest(MANIFEST):raise rb.GateError('manifest_changed_between_phases')
        if state.get('profile_name')!=PROFILE:raise rb.GateError('wrong_run_profile')
        return state,root
    def prepare_inputs(api,root,fixed):
        del api,fixed
        work=root/'work';work.mkdir();inputs=work/'inputs';inputs.mkdir();scripts=work/'recognizer_r1';scripts.mkdir()
    ...
    def docker_command(work,results,path,validator=False):
        ...
        command[index]='/work/recognizer_r1/wave_recognizer_r1_v1_runtime.py'
        return command
    ...
    if a.phase=='prepare':
        rb.prepare(PROFILE,profile);state,root=original_state();state['recognizer_r1_manifest_sha256']=digest(MANIFEST);rb.write_state(state)
...
if __name__=='__main__':
    try:main()
        ...
        print('Recognizer R1 stopped; consult bounded private evidence.',file=__import__('sys').stderr)
        raise SystemExit(1)
