"""Fixed recognizer R2 broker; only standard workflow_dispatch may call this."""
...
MANIFEST=HERE.parent/'docs/research/TWO_WAVE_RECOGNIZER_R2_EXECUTION_MANIFEST_V1.json'
SOURCES=(
 'wave_recognizer_r2_v1.py','wave_recognizer_r2_v1_entry.py','wave_recognizer_r2_v1_verifier.py','wave_recognizer_r2_v1_runtime.py','wave_recognizer_r2_v1_visuals.py','wave_recognizer_r2_v1_protocol.json',
 'wave_recognizer_r1_v1.py','wave_recognizer_r1_v1_visuals.py','wave_multiscale_dual_gates_v2.py','wave_dual_gate_hierarchy_v1.py','wave_dual_gate_probe_v1.py','two_wave_v0800_scale_map.py','two_wave_v0800_semantics.py')
COMMAND=['recognizer_r2/wave_recognizer_r2_v1_entry.py','--inputs','/work/inputs','--out','/results/study']
VERIFY=['recognizer_r2/wave_recognizer_r2_v1_verifier.py','--inputs','/work/inputs','--results','/results/study']
def digest(p):return hashlib.sha256(Path(p).read_bytes()).hexdigest()
def load_profile():
 if MANIFEST.is_symlink():raise ValueError('manifest symlink')
 m=json.loads(MANIFEST.read_text())
 if set(m)!={'schema_id','profile','private_ref','sources','new_training','production_authority'} or m['schema_id']!='csi1000.recognizer_r2_execution@1.0' or m['profile']!=PROFILE or m['private_ref']!=PRIVATE_REF or set(m['sources'])!=set(SOURCES):raise ValueError('unapproved manifest')
...
def publish(rb,profile):
 ...
 tag='public-research-run-'+state['run_id'];release=api.request(f'repos/{rb.PRIVATE_REPO}/releases',dict(tag_name=tag,target_commitish=PRIVATE_REF,draft=True,prerelease=True,name='Recognizer R2 '+state['run_id'],body='Pending bounded upload verification.'),method='POST')
 ...
 api.request(target,dict(message='Record bounded recognizer-r2 result [skip ci]',branch=state['branch'],content=base64.b64encode(payload).decode()),method='PUT');returned=api.request(target+'?ref='+urllib.parse.quote(state['branch'],safe=''))
 ...
 for name,raw in mirrors.items():
  mt=f'repos/{rb.PRIVATE_REPO}/contents/research/public-runs/{state["run_id"]}/{name}';api.request(mt,dict(message='Record fixed recognizer R2 readback [skip ci]',branch=state['branch'],content=base64.b64encode(raw).decode()),method='PUT');echo=api.request(mt+'?ref='+urllib.parse.quote(state['branch'],safe=''))
 ...
 print('Recognizer R2 private archive and receipt verified; no production authority.')
def main():
 ...
 def checked_state():
  ...
  if state.get('recognizer_r2_manifest_sha256')!=digest(MANIFEST):raise rb.GateError('manifest_changed_between_phases')
  if state.get('profile_name')!=PROFILE:raise rb.GateError('wrong_run_profile')
  return state,root
 def prepare_inputs(api,root,fixed):
  del api,fixed
  work=root/'work';work.mkdir();inputs=work/'inputs';inputs.mkdir();scripts=work/'recognizer_r2';scripts.mkdir()
 ...
 def docker_command(work,results,path,validator=False):
  command=original_docker(work,results,path,validator);command[command.index('/run_research_in_container.py')]='/work/recognizer_r2/wave_recognizer_r2_v1_runtime.py';return command
 ...
 if a.phase=='prepare':rb.prepare(PROFILE,profile);state,root=original_state();state['recognizer_r2_manifest_sha256']=digest(MANIFEST);rb.write_state(state)
...
if __name__=='__main__':
 try:main()
  ...
  print('Recognizer R2 stopped; consult bounded private evidence.',file=__import__('sys').stderr);raise SystemExit(1)
