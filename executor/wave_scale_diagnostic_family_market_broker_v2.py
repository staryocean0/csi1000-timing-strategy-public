"""Bounded standard broker for issue #381 label-blind diagnostic-family v2 measurement."""
...
ROOT=HERE.parent
PROFILE='two-wave-scale-diagnostic-family-measurement-v2'
PRIVATE_REF='7688ba57206dd29fbef88d8e57475255718471fe'
MANIFEST=ROOT/'docs/research/TWO_WAVE_SCALE_DIAGNOSTIC_FAMILY_MARKET_EXECUTION_MANIFEST_V2.json'
PROTOCOL=ROOT/'docs/research/TWO_WAVE_SCALE_DIAGNOSTIC_FAMILY_MARKET_PROTOCOL_V2_20260917.json'
...
COMMAND=['scale_diagnostic_family_v2/wave_scale_diagnostic_family_market_entry_v2.py']
VERIFY=['scale_diagnostic_family_v2/wave_scale_diagnostic_family_market_verifier_v2.py']
SOURCES=(
 'wave_scale_diagnostic_family_market_measurement_v2.py','wave_scale_diagnostic_family_market_entry_v2.py',
 'wave_scale_diagnostic_family_market_verifier_v2.py','wave_scale_diagnostic_family_v2.py',
 'wave_scale_validity_diagnostics_v1.py','wave_scale_dominance_diagnostics_v1.py',
 'wave_scale_validity_market_verifier_v1.py','wave_scale_dominance_market_verifier_v1.py',
 'wave_scale_reference_packet_v2.py','wave_scale_reference_sampling_v2.py','wave_scale_reference_visuals_v1.py',
 'wave_scale_reference_verifier_v2.py','wave_segmentation_carrier_qualification_v1.py','wave_segmentation_carrier_verifier_v1.py')
...
def load_manifest():
    ...
    required={'schema_id','profile','private_ref','source_blobs','dependency_blobs','broker_blob','stage_blob',
              'protocol_blob','carrier_protocol_blob','command','verify_command','command_timeout_seconds',
              'verification_timeout_seconds','new_training','production_authority'}
    if set(value)!=required or value['schema_id']!='csi1000.scale_diagnostic_family_market_execution@2.0':
    ...
    if blob(HERE/'wave_scale_diagnostic_family_market_broker_v2.py')!=value['broker_blob']:raise rb.GateError('broker_identity')
...
def verify_stage():
    ...
    if protocol['files']!=carrier['files'] or protocol['external_repository']!=carrier['external_repository'] or protocol['external_ref']!=carrier['external_ref']:
        raise rb.GateError('diagnostic_family_v2_carrier_identity')
...
def prepare_inputs(api,root,profile):
    ...
    inputs=work/'inputs';inputs.mkdir();scripts=work/'scale_diagnostic_family_v2';scripts.mkdir()
...
def publish(profile):
    ...
    state,root=rb.load_state();summary=root/'results'/'study'/'diagnostic_family_v2_summary.json'
    ...
    target=f"repos/{rb.PRIVATE_REPO}/contents/research/public-runs/{state['run_id']}/diagnostic_family_v2_summary.json"
    api.request(target,{'message':'Record diagnostic-family v2 aggregate [skip ci]',
    ...
    print('Diagnostic-family v2 aggregate privately mirrored and verified.')
