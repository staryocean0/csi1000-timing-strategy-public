"""Fixed entry for issue #381 label-blind diagnostic-family v2 measurement."""
...
import pandas as pd
from wave_scale_diagnostic_family_market_measurement_v2 import build_measurement, result_manifest
...
OUTPUTS = {'diagnostic_family_v2.jsonl', 'diagnostic_family_v2_summary.json'}
...
def load_protocol():
    ...
    if p.get('reference_labels_visible_to_compute') is not False or p.get('future_suffix_used') is not False:
        raise RuntimeError('blind_scope')
    if p.get('numeric_thresholds') is not None or p.get('threshold_selection_in_run') is not False:
        raise RuntimeError('threshold_scope')
    if p.get('R4_selected') or p.get('one_minute_strategy_admitted') or p.get('router_pnl') or p.get('production_authority'):
