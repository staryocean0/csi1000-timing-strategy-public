"""Bounded standard broker for issue #372 diagnostic-only market measurement."""
...
MANIFEST=ROOT/'docs/research/TWO_WAVE_SCALE_DOMINANCE_MARKET_EXECUTION_MANIFEST_V1.json'
PROTOCOL=ROOT/'docs/research/TWO_WAVE_SCALE_DOMINANCE_MARKET_PROTOCOL_20260917.json'
...
COMMAND=['scale_dominance_measurement/wave_scale_dominance_market_entry_v1.py']
VERIFY=['scale_dominance_measurement/wave_scale_dominance_market_verifier_v1.py']
SOURCES=(
 'wave_scale_dominance_market_measurement_v1.py','wave_scale_dominance_market_entry_v1.py',
 'wave_scale_dominance_market_verifier_v1.py','wave_scale_dominance_diagnostics_v1.py',
...
def load_manifest():
    ...
    if set(value)!=required or value['schema_id']!='csi1000.scale_dominance_market_execution@1.0':
    ...
    if blob(HERE/'wave_scale_dominance_market_broker_v1.py')!=value['broker_blob']:raise rb.GateError('broker_identity')
...
def verify_stage():
    ...
    if protocol['files']!=carrier['files'] or protocol['external_repository']!=carrier['external_repository'] or protocol['external_ref']!=carrier['external_ref']:
        raise rb.GateError('diagnostic_carrier_identity')
...
def prepare_inputs(api,root,profile):
    ...
    inputs=work/'inputs';inputs.mkdir();scripts=work/'scale_dominance_measurement';scripts.mkdir()
...
def publish(profile):
    ...
    state,root=rb.load_state();summary=root/'results'/'study'/'diagnostic_summary.json'
    ...
    target=f"repos/{rb.PRIVATE_REPO}/contents/research/public-runs/{state['run_id']}/diagnostic_summary.json"
    api.request(target,{'message':'Record scale-dominance diagnostic aggregate [skip ci]',
    ...
    print('Scale-dominance diagnostic aggregate privately mirrored and verified.')
