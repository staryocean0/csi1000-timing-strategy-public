"""Pure fixed-lag causal confirmation diagnostics for issue #386.

Target occurrence is relative minute 0. Knowledge time may extend to a frozen
positive lag, but candidate jump/turn locations are constrained to <= 0.
No labels, thresholds, outcomes, routing, or production authority.
"""
...
def _relative_axis(values, n):
    ...
    if 0 not in set(int(v) for v in r):
        raise ValueError("occurrence minute zero required")
    return r
...
def close_jump_confirmation(prices, relative_minutes):
    """Largest occurrence-time-or-earlier jump, confirmed through knowledge time."""
    ...
    if not len(eligible):
        raise ValueError("no occurrence-bounded jump candidate")
    move_index = int(eligible[np.argmax(absolute[eligible])])
...
def measure_validity_confirmation(base_prices, base_relative, base_phase_rows,
                                  knowledge_prices, knowledge_relative):
    base = measure_validity_v1(base_prices, base_relative, base_phase_rows)
    ext = close_jump_confirmation(knowledge_prices, knowledge_relative)
    return {
...
def _phase_close_rows(rows):
    ...
    if arr.ndim != 2 or arr.shape[1] != 2 or len(arr) < 2 * MIN_PIECEWISE_SIDE:
    ...
    if not np.any(rel <= 0):
        raise ValueError("occurrence-bounded phase support")
    prices = arr[:, 1]
    if np.any(prices <= 0):
...
def measure_morphology_phase_confirmation(rows):
    rel, prices = _phase_close_rows(rows)
    n = len(prices)
    eligible = [
        i for i in range(MIN_PIECEWISE_SIDE - 1, n - MIN_PIECEWISE_SIDE + 1)
        if rel[i] <= 0
    ]
    if len(eligible) < 2:
        raise ValueError("occurrence-bounded turn search support")
...
def measure_morphology_confirmation(phase_rows):
    if set(phase_rows) != set(PHASES):
        raise ValueError("exact five phase rows required")
    phases = {name: measure_morphology_phase_confirmation(phase_rows[name]) for name in PHASES}
    ...
    return {
        ...
        "phase_stability": {
            ...
            "reversal_completion_ratio": _quartiles([p["reversal_completion_ratio"] for p in phases.values()]),
            "post_turn_fraction": _quartiles([p["post_turn_fraction"] for p in phases.values()]),
            "turn_edge_distance_fraction": _quartiles([p["turn_edge_distance_fraction"] for p in phases.values()]),
            "delta_search_adjusted_bic_vs_one_leg": _quartiles([
                p["delta_search_adjusted_bic_vs_one_leg"] for p in phases.values()
            ]),
            "best_second_adjusted_bic_gap": _quartiles([
