"""Bounded standard broker for #359 blind reference packet generation."""
...
ROOT=HERE.parent
PROFILE='two-wave-scale-reference-blind-packet-v1'
...
COMMAND=['scale_reference_packet/wave_scale_reference_packet_entry_v1.py']
VERIFY=['scale_reference_packet/wave_scale_reference_packet_verifier_v1.py']
SOURCES=(
...
def load_manifest():
    ...
    if set(value)!=required or value['schema_id']!='csi1000.scale_reference_packet_execution@1.0':
    ...
    if blob(HERE/'wave_scale_reference_packet_broker_v1.py')!=value['broker_blob']:raise rb.GateError('broker_identity')
...
def publish(profile):
    ...
    api.request(target,{'message':'Record scale-reference packet aggregate [skip ci]',
