"""Build #359 blind reference packets from already-qualified carriers.
...
from wave_segmentation_carrier_qualification_v1 import (
    FILES, EXCLUDED, PRICE, canonicalize, complete_days, bucket_members,
)
from wave_scale_reference_sampling_v1 import (
    select_primary_days, blinded_inventory, normalize_log_shape,
)
...
def build_packet(raw_frames:dict[str,pd.DataFrame],declared_rows:dict[str,int])->dict[str,bytes]:
    ...
    full=select_primary_days(sorted(common))
    ...
    summary={
        'schema_id':'csi1000.scale_reference_blind_packet@1.0',
        ...
        'common_complete_eligible_days':len(common),'primary_panels':192,
...
def result_manifest(files:dict[str,bytes])->bytes:
    meta={name:{'bytes':len(raw),'sha256':hashlib.sha256(raw).hexdigest()}
          for name,raw in sorted(files.items())}
    return _encode({'schema_id':'csi1000.scale_reference_packet_manifest@1.0',
