"""Independent verifier for #359 real blind reference packet.
...
import wave_scale_reference_verifier_v1 as rv
...
def read_protocol():
    ...
    if p.get('schema_id')!='csi1000.scale_reference_packet_protocol@1.0':
        raise ValueError('protocol')
    if p.get('profile')!='two-wave-scale-reference-blind-packet-v1':
...
def anchor_context(sequence,row,length):
    ...
    if len(hits)!=1 or hits[0]-length+1<0:
...
def verify_file_manifest(manifest):
    if manifest.get('schema_id')!='csi1000.scale_reference_packet_manifest@1.0':
...
def verify_summary(summary):
    expected={'common_complete_eligible_days':1458,'primary_panels':192,'panel_files':576,
    ...
    if summary.get('schema_id')!='csi1000.scale_reference_blind_packet@1.0':raise ValueError('summary_schema')
...
def main():
    ...
    expected_full=rv.derive(sorted(common));expected_blind=rv.expected_blind(expected_full)
    ...
    rv.verify_inventories(sorted(common),full,blind)
    manifest=load_json(RESULTS/'manifest.json');actual=verify_file_manifest(manifest)
    summary=load_json(RESULTS/'packet_summary.json');verify_summary(summary)
