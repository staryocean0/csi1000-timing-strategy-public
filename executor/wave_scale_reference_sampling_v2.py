"""Candidate-independent reference sampling v2 amendment contracts for #363.
...
def blind_id(value: str, anchor: str) -> str:
    ...
    return hashlib.sha256(
        f"csi1000-s2-panel-v2|{value}|{anchor}".encode("utf-8")
    ).hexdigest()[:20]
...
def blinded_inventory(rows: list[dict]) -> list[dict]:
    ...
    return sorted(
        out,
        key=lambda r: _digest("csi1000-s2-review-order-v2|" + r["panel_id"]),
    )
