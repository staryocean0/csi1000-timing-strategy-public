...
def _rank(value):
    q=_quarter(value)
    return int.from_bytes(_digest(f'csi1000-s2-ref-v2|{q}|{value}'),'big')
...
def _panel(value,anchor):
    ...
    return hashlib.sha256(f'csi1000-s2-panel-v2|{value}|{anchor}'.encode()).hexdigest()[:20]
...
def expected_blind(full):
    ...
    return sorted(rows,key=lambda r:_digest('csi1000-s2-review-order-v2|'+r['panel_id']))
...
def verify_protocol(value):
    if value.get('schema_id')!='csi1000.scale_reference_protocol@2.0':raise ValueError('protocol')
