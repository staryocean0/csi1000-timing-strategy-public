"""Fixed scale-specific continuity broker. Only the standard workflow_dispatch may call this.
...
PROFILE='two-wave-scale-specific-continuity-v1'
PRIVATE_REF='7688ba57206dd29fbef88d8e57475255718471fe'
MANIFEST=HERE.parent/'docs/research/TWO_WAVE_SCALE_SPECIFIC_CONTINUITY_EXECUTION_MANIFEST_V1.json'
SOURCES=('wave_scale_specific_continuity_v1.py','wave_scale_specific_continuity_v1_entry.py','wave_scale_specific_continuity_v1_verifier.py','wave_scale_specific_continuity_v1_runtime.py','wave_multiscale_dual_gates_v2.py','wave_dual_gate_hierarchy_v1.py','wave_dual_gate_probe_v1.py','two_wave_v0800_scale_map.py','two_wave_v0800_semantics.py')
COMMAND=['scale_continuity/wave_scale_specific_continuity_v1_entry.py','--inputs','/work/inputs','--out','/results/study']
VERIFY=['scale_continuity/wave_scale_specific_continuity_v1_verifier.py','--inputs','/work/inputs','--results','/results/study']
...
def load_profile():
    ...
    if set(m)!={'schema_id','profile','private_ref','sources','new_training','production_authority'} or m['schema_id']!='csi1000.scale_specific_continuity_execution@1.0' or m['profile']!=PROFILE or m['private_ref']!=PRIVATE_REF or set(m['sources'])!=set(SOURCES):
...
def publish(rb,profile):
    ...
    release=api.request(f'repos/{rb.PRIVATE_REPO}/releases',dict(tag_name=tag,target_commitish=PRIVATE_REF,draft=True,prerelease=True,name='Scale-specific continuity v1 '+state['run_id'],body='Pending bounded upload verification.'),method='POST')
    ...
    api.request(target,dict(message='Record bounded multiscale dual-gate v2 result [skip ci]',branch=state['branch'],content=base64.b64encode(payload).decode()),method='PUT')
    ...
    for name,raw in mirrors.items():
        ...
        api.request(mirror_target,dict(message='Record fixed scale-specific continuity readback [skip ci]',branch=state['branch'],content=base64.b64encode(raw).decode()),method='PUT')
    ...
    print('Scale-specific continuity v1 private archive and receipt verified; no production authority.')
...
def main():
    ...
    def checked_state():
        ...
        if state.get('scale_specific_continuity_manifest_sha256')!=digest(MANIFEST):raise rb.GateError('manifest_changed_between_phases')
        if state.get('profile_name')!=PROFILE:raise rb.GateError('wrong_run_profile')
        return state,root
    def prepare_inputs(api,root,fixed):
        del api,fixed
        work=root/'work';work.mkdir();inputs=work/'inputs';inputs.mkdir();scripts=work/'scale_continuity';scripts.mkdir()
    ...
    def docker_command(work,results,path,validator=False):
        ...
        command[index]='/work/scale_continuity/wave_scale_specific_continuity_v1_runtime.py'
        return command
    ...
    if a.phase=='prepare':
        rb.prepare(PROFILE,profile);state,root=original_state();state['scale_specific_continuity_manifest_sha256']=digest(MANIFEST);rb.write_state(state)
...
if __name__=='__main__':
    try:main()
        ...
        print('Scale-specific continuity v1 stopped; consult bounded private evidence.',file=__import__('sys').stderr)
        raise SystemExit(1)
