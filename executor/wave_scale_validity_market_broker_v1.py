"""Bounded standard broker for issue #376 label-blind validity measurement."""
...
COMMAND=['scale_validity_measurement/wave_scale_validity_market_entry_v1.py']
VERIFY=['scale_validity_measurement/wave_scale_validity_market_verifier_v1.py']
SOURCES=(
 'wave_scale_validity_market_measurement_v1.py','wave_scale_validity_market_entry_v1.py',
 'wave_scale_validity_market_verifier_v1.py','wave_scale_validity_diagnostics_v1.py',
...
def load_manifest():
    ...
    if set(value)!=required or value['schema_id']!='csi1000.scale_validity_market_execution@1.0':
    ...
    if blob(HERE/'wave_scale_validity_market_broker_v1.py')!=value['broker_blob']:raise rb.GateError('broker_identity')
...
def verify_stage():
    ...
    if protocol['files']!=carrier['files'] or protocol['external_repository']!=carrier['external_repository'] or protocol['external_ref']!=carrier['external_ref']:
        raise rb.GateError('validity_carrier_identity')
...
def prepare_inputs(api,root,profile):
    ...
    inputs=work/'inputs';inputs.mkdir();scripts=work/'scale_validity_measurement';scripts.mkdir()
...
def publish(profile):
    ...
    state,root=rb.load_state();summary=root/'results'/'study'/'validity_summary.json'
    ...
    target=f"repos/{rb.PRIVATE_REPO}/contents/research/public-runs/{state['run_id']}/validity_summary.json"
    api.request(target,{'message':'Record scale-validity diagnostic aggregate [skip ci]',
    ...
    print('Scale-validity diagnostic aggregate privately mirrored and verified.')
