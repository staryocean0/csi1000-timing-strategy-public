"""Fixed no-argument entry for issue #376 label-blind validity measurement."""
from __future__ import annotations
import hashlib,json
from pathlib import Path
import pandas as pd
from wave_scale_validity_market_measurement_v1 import build_validity_measurement, result_manifest

INPUTS=Path('/work/inputs');OUT=Path('/results/study')
PROTOCOL=Path('/work/scale_validity_measurement/protocol.json')
FILES=('1m_official.parquet',*(f'5m_offset_{i}.parquet' for i in range(5)))
OUTPUTS={'validity_diagnostics.jsonl','validity_summary.json'}
...
def load_protocol():
    ...
    if p.get('schema_id')!='csi1000.scale_validity_market_protocol@1.0':raise RuntimeError('protocol_identity')
    if p.get('profile')!='two-wave-scale-validity-diagnostic-measurement-v1':raise RuntimeError('profile_identity')
    ...
    if p.get('numeric_validity_thresholds') is not None or p.get('threshold_selection_in_run') is not False:raise RuntimeError('threshold_scope')
...
def main():
    ...
    protocol=load_protocol();frames,declared=load_inputs(protocol);files=build_validity_measurement(frames,declared)
    ...
    print(json.dumps({'status':'passed','panels':192,'reference_labels_joined':False,'validity_state_assigned':False,'threshold_selected':False,'production_authority':False},sort_keys=True,separators=(',',':')))
