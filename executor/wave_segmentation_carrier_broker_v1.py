"""Bounded public-data broker for issue #352 carrier qualification."""
...
COMMAND=['segmentation_carrier/wave_segmentation_carrier_entry_v1.py']
VERIFY=['segmentation_carrier/wave_segmentation_carrier_verifier_v1.py']
SOURCES=('wave_segmentation_carrier_qualification_v1.py','wave_segmentation_carrier_entry_v1.py','wave_segmentation_carrier_verifier_v1.py')
...
def load_manifest()->dict:
    ...
    required={'schema_id','profile','private_ref','source_blobs','dependency_blobs','broker_blob','stage_blob','protocol_blob','command','verify_command',
              'command_timeout_seconds','verification_timeout_seconds','new_training','production_authority'}
    if set(value)!=required or value['schema_id']!='csi1000.segmentation_carrier_execution@1.0':raise rb.GateError('execution_manifest_invalid')
    ...
    if value['command_timeout_seconds']!=300 or value['verification_timeout_seconds']!=300:raise rb.GateError('timeout_identity')
    ...
    for name,expected in value['source_blobs'].items():
        path=HERE/name
        if path.is_symlink() or not path.is_file() or blob(path)!=expected:raise rb.GateError('source_identity')
    deps={'research_broker.py':'1b4968475fc91a34f53bc08b7b5fdc1bf79862c0','broker.py':'299ab90424e42f76f038d3bad719cb2abeb155dc','run_research_in_container.py':'38dfa9bbcf0a09b328495b4844c82e25c9b5bec8'}
    ...
    if blob(HERE/'wave_segmentation_carrier_broker_v1.py')!=value['broker_blob']:raise rb.GateError('broker_identity')
    if blob(HERE/'wave_segmentation_carrier_stage_public.py')!=value['stage_blob']:raise rb.GateError('stage_identity')
    if blob(PROTOCOL)!=value['protocol_blob']:raise rb.GateError('protocol_identity')
    return value
...
def load_profile(name):
    ...
    return {'private_ref':PRIVATE_REF,'manifest_sha256':digest(MANIFEST),'command':COMMAND,'verify_command':VERIFY,
            'command_timeout_seconds':300,'verification_timeout_seconds':300,'new_training':False,'production_authority':False}


def verify_stage()->tuple[Path,dict]:
    ...
    try:receipt=json.loads(receipt_path.read_text());protocol=json.loads(PROTOCOL.read_text())
    ...
    if receipt.get('private_credentials_used') is not False or receipt.get('repository')!=protocol['external_repository'] or receipt.get('ref')!=protocol['external_ref']:
        raise rb.GateError('public_stage_identity')
    if set(receipt.get('files',{}))!=set(protocol['files']):raise rb.GateError('public_stage_file_set')
    for name,meta in protocol['files'].items():
        ...
        if got!={'bytes':meta['bytes'],'sha256':meta['sha256'],'git_blob_sha1':meta['git_blob_sha1']}:
...
def prepare_inputs(api,root,profile):
    del api,profile
    staged,protocol=verify_stage();work=root/'work';work.mkdir();inputs=work/'inputs';inputs.mkdir();scripts=work/'segmentation_carrier';scripts.mkdir()
...
def publish(profile):
    ...
    state,root=rb.load_state();report=root/'results'/'study'/'report.json'
    if report.is_symlink() or not report.is_file() or report.stat().st_size>1024*1024:raise rb.GateError('aggregate_readback_bound')
    raw=report.read_bytes();api=rb.require_private_api()
    target=f"repos/{rb.PRIVATE_REPO}/contents/research/public-runs/{state['run_id']}/report.json"
    api.request(target,{'message':'Record segmentation carrier qualification aggregate [skip ci]',
    ...
    print('Segmentation carrier qualification aggregate privately mirrored and verified.')


rb.PROFILE_NAME=PROFILE
rb.COMMAND=COMMAND
rb.VERIFY_COMMAND=VERIFY
rb.COMMAND_TIMEOUT_SECONDS=300
rb.VERIFICATION_TIMEOUT_SECONDS=300
rb.COMPUTE_HOST_TIMEOUT_SECONDS=330
