...
def strip_issue635_workflow(text: str) -> str:
    ...
    if text.count(condition) != 2:
        raise AssertionError("issue635 allowlist count")
    text = text.replace(condition, "")
    for phase in ("prepare", "compute", "cleanup", "publish"):
        branch = (
            f"          elif [ '${{{{ inputs.profile }}}}' = '{ISSUE635_PROFILE}' ]; then\n"
            f"            python3 {ISSUE635_BROKER} {phase} {ISSUE635_PROFILE}\n"
        )
        text = remove_once(text, branch, "issue635 " + phase)
    return text


def strip_issue624_workflow(text: str) -> str:
    text = strip_issue635_workflow(text)
...
def strip_issue615_workflow(text: str) -> str:
    ...
    if text.count(condition) != 2:
        raise AssertionError("issue615 allowlist count")
    text = text.replace(condition, "")
    for phase in ("prepare", "compute", "cleanup", "publish"):
        branch = (
            f"          elif [ '${{{{ inputs.profile }}}}' = '{ISSUE615_PROFILE}' ]; then\n"
            f"            python3 {ISSUE615_BROKER} {phase} {ISSUE615_PROFILE}\n"
        )
        text = remove_once(text, branch, "issue615 " + phase)
    return text


def strip_fixed_lag_workflow(text: str) -> str:
    text = strip_issue615_workflow(text)
