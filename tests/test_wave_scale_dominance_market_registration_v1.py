...
class RegistrationTests(unittest.TestCase):
 ...
 def test_history_guard_prior24(self):
  x=json.loads((ROOT/'docs/research/TWO_WAVE_RESEARCH_LINEAGE_20260916.json').read_text());self.assertEqual(len(x['ordered_lineage']),25)
 ...
 def test_strip_public_workflow_restores_exact_parent_blob(self):
  s=(ROOT/'.github/workflows/public-compute.yml').read_text();s=s.replace(f"          - {PROFILE}\n",'');s=s.replace(f" || inputs.profile == '{PROFILE}'",'')
 ...
 def test_strip_controller_restores_exact_parent_blob(self):
  s=(ROOT/'.github/workflows/controller-dispatch.yml').read_text();s=s.replace(f"       github.event.issue.title == 'controller: {PROFILE}' ||\n",'')
 ...
 def test_authority_advances_only_to_not_run_profile(self):
  ...
  self.assertEqual(lane['status'],'DIAGNOSTIC_MEASUREMENT_PROFILE_SOURCE_READY_NOT_RUN');self.assertFalse(a['S2_progress']['diagnostic_scores_measured']);self.assertIsNone(a['S2_progress']['market_thresholds'])
