...
class ReferenceAnnotationAdjudicationTests(unittest.TestCase):
    ...
    def test_authority_moves_to_diagnostic_measurement_only(self):
        ...
        self.assertIn(lane['status'],{'PRIMARY_REFERENCE_LABELS_FROZEN_DIAGNOSTIC_MEASUREMENT_REGISTRATION_NEXT','DIAGNOSTIC_MEASUREMENT_PROFILE_SOURCE_READY_NOT_RUN','DIAGNOSTIC_MEASUREMENT_AND_THRESHOLD_FREE_JOIN_COMPLETED_CALIBRATION_GATE_NEXT'})
        self.assertIn(self.u['S2_progress']['next'],{'REGISTER_POST_REFERENCE_DIAGNOSTIC_ONLY_MARKET_MEASUREMENT','MERGE_PROFILE_THEN_FORMAL_DIAGNOSTIC_ONLY_MARKET_MEASUREMENT','REGISTER_STATE_MORPHOLOGY_CONDITIONAL_CALIBRATION_AND_VALIDITY_GUARD_GATE'})
