...
class ReferenceAnnotationAdjudicationTests(unittest.TestCase):
    ...
    def test_authority_moves_to_diagnostic_measurement_only(self):
        ...
        self.assertIn(lane['status'],{'PRIMARY_REFERENCE_LABELS_FROZEN_DIAGNOSTIC_MEASUREMENT_REGISTRATION_NEXT','DIAGNOSTIC_MEASUREMENT_PROFILE_SOURCE_READY_NOT_RUN','DIAGNOSTIC_MEASUREMENT_AND_THRESHOLD_FREE_JOIN_COMPLETED_CALIBRATION_GATE_NEXT','VALIDITY_DIAGNOSTIC_PROFILE_SOURCE_READY_NOT_RUN','VALIDITY_MEASUREMENT_PASSED_CALIBRATION_ENGINE_SOURCE_READY_NOT_FIT','FIRST_REAL_LOYO_FAILED_DIAGNOSTIC_FAMILY_REVISION_REQUIRED','DIAGNOSTIC_FAMILY_V2_PROFILE_SOURCE_READY_NOT_RUN'})
        self.assertIn(self.u['S2_progress']['next'],{'REGISTER_POST_REFERENCE_DIAGNOSTIC_ONLY_MARKET_MEASUREMENT','MERGE_PROFILE_THEN_FORMAL_DIAGNOSTIC_ONLY_MARKET_MEASUREMENT','REGISTER_STATE_MORPHOLOGY_CONDITIONAL_CALIBRATION_AND_VALIDITY_GUARD_GATE','MERGE_VALIDITY_PROFILE_THEN_FORMAL_LABEL_BLIND_MEASUREMENT','MERGE_CALIBRATION_ENGINE_THEN_RUN_FROZEN_LOYO_OOF','PREREGISTER_DIAGNOSTIC_FAMILY_REVISION_V2','MERGE_DIAGNOSTIC_FAMILY_V2_PROFILE_THEN_FORMAL_LABEL_BLIND_MEASUREMENT'})
