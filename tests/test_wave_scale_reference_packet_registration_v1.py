...
from tests.segmentation_carrier_registration_support import (
    ...
    PACKET_BROKER as BROKER, PACKET_V2_PROFILE,PACKET_V3_PROFILE,MARKET_PROFILE,VALIDITY_PROFILE,DIAG_V2_PROFILE,FIXED_LAG_PROFILE, strip_packet_workflow, strip_packet_controller,
)
...
class PacketRegistrationTests(unittest.TestCase):
    ...
    def test_profile_option_and_shared_public_stage(self):
        ...
        self.assertEqual(stage[0]['if'],f"inputs.profile == '{CARRIER_PROFILE}' || inputs.profile == '{PROFILE}' || inputs.profile == '{PACKET_V2_PROFILE}' || inputs.profile == '{PACKET_V3_PROFILE}' || inputs.profile == '{MARKET_PROFILE}' || inputs.profile == '{VALIDITY_PROFILE}' || inputs.profile == '{DIAG_V2_PROFILE}' || inputs.profile == '{FIXED_LAG_PROFILE}'")
