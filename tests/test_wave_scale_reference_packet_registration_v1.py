...
from tests.segmentation_carrier_registration_support import (
    ...
    PACKET_BROKER as BROKER, strip_packet_workflow, strip_packet_controller,
)
...
class PacketRegistrationTests(unittest.TestCase):
    ...
    def test_profile_option_and_shared_public_stage(self):
        ...
        self.assertEqual(stage[0]['if'],f"inputs.profile == '{CARRIER_PROFILE}' || inputs.profile == '{PROFILE}'")
    ...
    def test_thread_authority_is_registered_not_run(self):
        ...
        self.assertEqual(lane['status'],'BLIND_PACKET_PROFILE_REGISTERED_MARKET_PACKET_RUN_NEXT')
        self.assertEqual(a['S2_progress']['market_packet_profile'],'REGISTERED_NOT_RUN')
        self.assertIsNone(a['S2_progress']['market_packet_run'])
