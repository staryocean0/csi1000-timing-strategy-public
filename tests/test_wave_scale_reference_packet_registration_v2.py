...
from tests.segmentation_carrier_registration_support import (
    ...
    PACKET_V2_PROFILE as PROFILE, PACKET_V2_BROKER as BROKER, PACKET_V3_PROFILE, MARKET_PROFILE, VALIDITY_PROFILE,
...
class PacketV2RegistrationTests(unittest.TestCase):
    ...
    def test_profile_stage_and_secret_boundary(self):
        ...
        self.assertEqual(stage['if'],f"inputs.profile == '{CARRIER_PROFILE}' || inputs.profile == '{V1_PROFILE}' || inputs.profile == '{PROFILE}' || inputs.profile == '{PACKET_V3_PROFILE}' || inputs.profile == '{MARKET_PROFILE}' || inputs.profile == '{VALIDITY_PROFILE}'")
