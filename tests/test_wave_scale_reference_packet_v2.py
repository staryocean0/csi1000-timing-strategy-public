...
import wave_scale_reference_packet_v2 as p
import wave_scale_reference_packet_verifier_v2 as v
import wave_scale_reference_sampling_v2 as s
...
class ReferencePacketTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.eligible,cls.exclusions=p.anchor_context_eligible_days(cls.frames,cls.sequence,cls.common)
        cls.rows=s.select_primary_days(sorted(cls.eligible))
    ...
    def test_flat_fill_is_excluded_before_hash_ranking(self):
        ...
        eligible,reasons=p.anchor_context_eligible_days(self.frames,bad,self.common)
        self.assertNotIn(row['day'],eligible)
        self.assertGreater(reasons.get('context_causal_flat_fill',0),0)
        reranked=s.select_primary_days(sorted(eligible))
        self.assertEqual(len(reranked),192)
        self.assertNotIn(row['day'],{x['day'] for x in reranked})

    def test_producer_and_independent_pre_rank_eligibility_agree(self):
        eligible,reasons=v.anchor_context_eligible_days(self.sequence,self.common)
        self.assertEqual(eligible,self.eligible)
        self.assertEqual(reasons,self.exclusions)
    ...
    def test_verifier_is_independent_from_producer_and_renderer(self):
        ...
        self.assertNotIn('wave_scale_reference_packet_v2',text)
        self.assertNotIn('wave_scale_reference_packet_entry_v2',text)
    ...
    def test_producer_has_no_detector_or_dominance_import(self):
        text=(ROOT/'executor/wave_scale_reference_packet_v2.py').read_text()
        self.assertNotIn('wave_recognizer_r',text)
        self.assertNotIn('wave_scale_dominance_diagnostics',text)
        self.assertNotIn('outcome_',text.lower())

    def test_entry_is_no_argument_fixed_path(self):
        text=(ROOT/'executor/wave_scale_reference_packet_entry_v2.py').read_text()
    ...
    def test_packet_protocol_pins_reference_without_thresholds(self):
        protocol=json.loads((ROOT/'docs/research/TWO_WAVE_SCALE_REFERENCE_PACKET_PROTOCOL_V2_20260917.json').read_text())
        self.assertEqual(protocol['profile'],'two-wave-scale-reference-blind-packet-v2')
        self.assertEqual(set(protocol['files']),set(p.FILES))
        self.assertEqual(protocol['reference_contract']['primary_panels'],192)
        self.assertTrue(protocol['reference_contract']['eligibility_before_hash_ranking'])
        self.assertIn('csi1000-s2-ref-v2',protocol['reference_contract']['day_rank'])
